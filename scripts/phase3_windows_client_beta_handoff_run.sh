...
Purpose:
  One-command Phase-3 Windows client-beta handoff runner:
    1) phase3_windows_client_beta_run.sh
    2) phase3_windows_client_beta_handoff_check.sh

Notes:
  - Wrapper-owned flags are reserved; stage pass-through uses prefixes:
      --run-...      -> forwarded to phase3_windows_client_beta_run.sh
      --handoff-...  -> forwarded to phase3_windows_client_beta_handoff_check.sh
...
run_summary_contract_valid() {
  ...
  jq -e '
    ...
    and (.rc | type) == "number"
    and (.steps.ci_phase3_windows_client_beta | type) == "object"
    and (.steps.phase3_windows_client_beta_check | type) == "object"
...
run_stage_capture() {
  ...
  shift 2
  local rc=0
  echo "[phase3-windows-client-beta-handoff-run] stage=$label status=running"
  set +e
  "$@" >"$log_path" 2>&1
  rc=$?
  if (( rc == 0 )); then
    echo "[phase3-windows-client-beta-handoff-run] stage=$label status=pass rc=0"
  else
    echo "[phase3-windows-client-beta-handoff-run] stage=$label status=fail rc=$rc"
  fi
  return "$rc"
}
...
extract_roadmap_summary_path() {
  ...
  if [[ -f "$run_summary_json" ]] && jq -e . "$run_summary_json" >/dev/null 2>&1; then
    path="$(jq -r '(.steps.phase3_windows_client_beta_check.artifacts.roadmap_summary_json // .artifacts.roadmap_summary_json // empty)' "$run_summary_json" 2>/dev/null || true)"
  fi
  if [[ -n "$path" ]]; then
...
reports_dir="${PHASE3_WINDOWS_CLIENT_BETA_HANDOFF_RUN_REPORTS_DIR:-}"
run_summary_json="${PHASE3_WINDOWS_CLIENT_BETA_HANDOFF_RUN_RUN_SUMMARY_JSON:-}"
handoff_summary_json="${PHASE3_WINDOWS_CLIENT_BETA_HANDOFF_RUN_HANDOFF_SUMMARY_JSON:-}"
summary_json="${PHASE3_WINDOWS_CLIENT_BETA_HANDOFF_RUN_SUMMARY_JSON:-}"
print_summary_json="${PHASE3_WINDOWS_CLIENT_BETA_HANDOFF_RUN_PRINT_SUMMARY_JSON:-1}"
resume="${PHASE3_WINDOWS_CLIENT_BETA_HANDOFF_RUN_RESUME:-0}"
dry_run="${PHASE3_WINDOWS_CLIENT_BETA_HANDOFF_RUN_DRY_RUN:-0}"
run_phase3_windows_client_beta_run="${PHASE3_WINDOWS_CLIENT_BETA_HANDOFF_RUN_RUN_PHASE3_WINDOWS_CLIENT_BETA_RUN:-1}"
run_phase3_windows_client_beta_handoff_check="${PHASE3_WINDOWS_CLIENT_BETA_HANDOFF_RUN_RUN_PHASE3_WINDOWS_CLIENT_BETA_HANDOFF_CHECK:-1}"
...
run_script="${PHASE3_WINDOWS_CLIENT_BETA_HANDOFF_RUN_RUN_SCRIPT:-$ROOT_DIR/scripts/phase3_windows_client_beta_run.sh}"
handoff_check_script="${PHASE3_WINDOWS_CLIENT_BETA_HANDOFF_RUN_HANDOFF_CHECK_SCRIPT:-$ROOT_DIR/scripts/phase3_windows_client_beta_handoff_check.sh}"

if [[ "$run_phase3_windows_client_beta_run" == "1" && ! -x "$run_script" ]]; then
  echo "missing executable stage script: $run_script"
  exit 2
fi
if [[ "$run_phase3_windows_client_beta_handoff_check" == "1" && ! -x "$handoff_check_script" ]]; then
...
if [[ -z "$reports_dir" ]]; then
  reports_dir="$ROOT_DIR/.easy-node-logs/phase3_windows_client_beta_handoff_run_${run_stamp}"
else
  reports_dir="$(abs_path "$reports_dir")"
fi
if [[ -z "$run_summary_json" ]]; then
  run_summary_json="$reports_dir/phase3_windows_client_beta_run_summary.json"
else
  run_summary_json="$(abs_path "$run_summary_json")"
fi
if [[ -z "$handoff_summary_json" ]]; then
  handoff_summary_json="$reports_dir/phase3_windows_client_beta_handoff_check_summary.json"
else
  handoff_summary_json="$(abs_path "$handoff_summary_json")"
fi
if [[ -z "$summary_json" ]]; then
  summary_json="$reports_dir/phase3_windows_client_beta_handoff_run_summary.json"
else
...
if [[ "$run_phase3_windows_client_beta_run" == "1" ]]; then
  if [[ "$resume" == "1" ]] && summary_pass_contract_valid "$run_summary_json" run_summary_contract_valid; then
    echo "[phase3-windows-client-beta-handoff-run] stage=phase3_windows_client_beta_run status=pass rc=0 reason=resume-artifact-pass"
    run_command_rc=0
    ...
    set +e
    run_stage_capture "phase3_windows_client_beta_run" "$run_log" "${run_cmd[@]}"
    run_command_rc=$?
    set -e
  ...
  echo "[phase3-windows-client-beta-handoff-run] stage=phase3_windows_client_beta_run status=skipped reason=disabled"
fi

declare -a handoff_cmd=(
  "$handoff_check_script"
  --phase3-run-summary-json "$run_summary_json"
...
if [[ "$dry_run" == "1" ]]; then
  ...
  if ! array_has_arg "--require-desktop-scaffold-ok" "${handoff_cmd[@]:1}"; then
    handoff_cmd+=(--require-desktop-scaffold-ok 0)
  fi
  if ! array_has_arg "--require-local-control-api-ok" "${handoff_cmd[@]:1}"; then
    handoff_cmd+=(--require-local-control-api-ok 0)
  fi
  if ! array_has_arg "--require-local-api-config-defaults-ok" "${handoff_cmd[@]:1}"; then
    handoff_cmd+=(--require-local-api-config-defaults-ok 0)
  fi
  if ! array_has_arg "--require-easy-node-config-v1-ok" "${handoff_cmd[@]:1}"; then
    handoff_cmd+=(--require-easy-node-config-v1-ok 0)
  fi
  if ! array_has_arg "--require-launcher-wiring-ok" "${handoff_cmd[@]:1}"; then
    handoff_cmd+=(--require-launcher-wiring-ok 0)
  fi
  if ! array_has_arg "--require-launcher-runtime-ok" "${handoff_cmd[@]:1}"; then
    handoff_cmd+=(--require-launcher-runtime-ok 0)
  fi
fi
handoff_command="$(print_cmd "${handoff_cmd[@]}")"

if [[ "$run_phase3_windows_client_beta_handoff_check" == "1" ]]; then
  if [[ "$resume" == "1" ]] && summary_pass_contract_valid "$handoff_summary_json" handoff_summary_contract_valid; then
    echo "[phase3-windows-client-beta-handoff-run] stage=phase3_windows_client_beta_handoff_check status=pass rc=0 reason=resume-artifact-pass"
    ...
    set +e
    run_stage_capture "phase3_windows_client_beta_handoff_check" "$handoff_log" "${handoff_cmd[@]}"
    handoff_command_rc=$?
    set -e
    if handoff_summary_contract_valid "$handoff_summary_json"; then
      ...
      handoff_failure_kind="$(jq -r '.failure.kind // ""' "$handoff_summary_json" 2>/dev/null || true)"
  ...
  echo "[phase3-windows-client-beta-handoff-run] stage=phase3_windows_client_beta_handoff_check status=skipped reason=disabled"
fi

final_rc=0
if [[ "$run_phase3_windows_client_beta_run" == "1" ]] && (( run_rc != 0 )) && (( final_rc == 0 )); then
  final_rc="$run_rc"
fi
if [[ "$run_phase3_windows_client_beta_handoff_check" == "1" ]] && (( handoff_rc != 0 )) && (( final_rc == 0 )); then
  final_rc="$handoff_rc"
fi
...
jq -n \
  ...
  --argjson run_phase3_windows_client_beta_run "$run_phase3_windows_client_beta_run" \
  --argjson run_phase3_windows_client_beta_handoff_check "$run_phase3_windows_client_beta_handoff_check" \
  ...
  --arg handoff_failure_kind "$handoff_failure_kind" \
  ...
  '{
    ...
    metadata: {
      ...
      runner_script: "phase3_windows_client_beta_handoff_run.sh"
    },
    inputs: {
    ...
    steps: {
      phase3_windows_client_beta_run: {
        enabled: ($run_phase3_windows_client_beta_run == 1),
        status: $run_status,
        rc: $run_rc,
      ...
      phase3_windows_client_beta_handoff_check: {
        enabled: ($run_phase3_windows_client_beta_handoff_check == 1),
        ...
        failure: {
          kind: (if $handoff_failure_kind == "" then null else $handoff_failure_kind end),
          policy_no_go: ($handoff_failure_kind == "policy_no_go"),
          execution_failure: ($handoff_failure_kind == "execution_failure"),
          timeout: ($handoff_failure_kind == "timeout")
        },
        policy_outcome: {
          decision: (if $handoff_policy_outcome_decision == "" then null else $handoff_policy_outcome_decision end),
          fail_closed_no_go: ($handoff_failure_kind == "policy_no_go")
        },
        actionable: {
