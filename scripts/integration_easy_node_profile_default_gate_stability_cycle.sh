...
FAKE_SCRIPT="$TMP_DIR/fake_profile_default_gate_stability_cycle.sh"

cat >"$FAKE_SCRIPT" <<'EOF_FAKE'
#!/usr/bin/env bash
set -euo pipefail
capture_file="${PROFILE_DEFAULT_GATE_STABILITY_CYCLE_CAPTURE_FILE:?}"
{
  printf 'argc=%s' "$#"
  for arg in "$@"; do
    printf '\t%s' "$arg"
...
echo "fake profile default gate stability cycle: $*"
exit "${FAKE_PROFILE_DEFAULT_GATE_STABILITY_CYCLE_RC:-0}"
EOF_FAKE
chmod +x "$FAKE_SCRIPT"
...
echo "[easy-node-profile-default-gate-stability-cycle] help contract"
bash "$SCRIPT_UNDER_TEST" help >"$HELP_OUT"
if ! grep -F -- './scripts/easy_node.sh profile-default-gate-stability-cycle --host-a HOST --host-b HOST [--campaign-subject ID|--subject ID] [--runs N] [--campaign-timeout-sec N] [--campaign-runs N] [--campaign-profile-rounds N] [--campaign-profile-timeout-sec N] [--campaign-require-min-runs-total N] [--campaign-require-min-runs-with-summary N] [--campaign-require-trend-source CSV] [--campaign-execution-mode docker|local] [--campaign-live-evidence [0|1]] [--require-external-live-evidence [0|1]] [--campaign-live-evidence-udp-inject [0|1]] [--allow-remote-http-probe [0|1]] [--sleep-between-sec N] [--reports-dir DIR] [--stability-summary-json PATH] [--stability-check-summary-json PATH] [--summary-json PATH] [--print-summary-json [0|1]] [--fail-on-no-go [0|1]]' "$HELP_OUT" >/dev/null 2>&1; then
  echo "easy_node help missing profile-default-gate-stability-cycle command contract"
  cat "$HELP_OUT"
  exit 1
fi

echo "[easy-node-profile-default-gate-stability-cycle] forwarding contract"
: >"$CAPTURE"
PROFILE_DEFAULT_GATE_STABILITY_CYCLE_SCRIPT="$FAKE_SCRIPT" \
PROFILE_DEFAULT_GATE_STABILITY_CYCLE_CAPTURE_FILE="$CAPTURE" \
bash "$SCRIPT_UNDER_TEST" profile-default-gate-stability-cycle \
  ...
  --sleep-between-sec 3 \
  --reports-dir .easy-node-logs/stability_cycle_contract \
  --stability-summary-json .easy-node-logs/profile_default_gate_stability_summary.json \
  --stability-check-summary-json .easy-node-logs/profile_default_gate_stability_check_summary.json \
...
assert_token "$line" $'\t--runs\t5' "missing --runs forwarding"
...
assert_token "$line" $'\t--reports-dir\t.easy-node-logs/stability_cycle_contract' "missing --reports-dir forwarding"
...
assert_token "$line" $'\t--stability-check-summary-json\t.easy-node-logs/profile_default_gate_stability_check_summary.json' "missing --stability-check-summary-json forwarding"
assert_token "$line" $'\t--summary-json\t.easy-node-logs/profile_default_gate_stability_cycle_summary.json' "missing --summary-json forwarding"
...
if ! grep -F -- 'fake profile default gate stability cycle:' "$STDOUT_OUT" >/dev/null 2>&1; then
  echo "missing wrapper output from fake script"
  cat "$STDOUT_OUT"
  exit 1
fi

echo "easy node profile-default-gate-stability-cycle integration check ok"
