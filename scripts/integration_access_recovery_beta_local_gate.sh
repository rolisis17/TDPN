...
if ! jq -e '
  ...
  and ((.recommended_next_action.command // "") | test("access-bridge-pilot-evidence-bundle-verify"))
  and ((.recommended_next_action.command // "") | test("--trust-store TRUST_STORE"))
  and ((.recommended_next_action.command // "") | test("--require-trusted-provenance 1"))
  and ((.recommended_next_action.command // "") | test("--verification-summary-json"))
  and ((.recommended_next_action.command // "") | test("--print-verification-summary-json 1"))
  and ([.steps[].id] == [
    "demo_contract",
    "examples_contract",
    "browser_smoke",
