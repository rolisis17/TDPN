...
FAKE_SCRIPT="$TMP_DIR/fake_roadmap_live_and_pack_actionable_run.sh"

cat >"$FAKE_SCRIPT" <<'EOF_FAKE'
#!/usr/bin/env bash
set -euo pipefail

capture_file="${ROADMAP_LIVE_AND_PACK_ACTIONABLE_CAPTURE_FILE:?}"
{
  printf 'argc=%s' "$#"
  for arg in "$@"; do
    printf '\t%s' "$arg"
...
echo "fake roadmap live-and-pack actionable run: $*"
exit "${FAKE_ROADMAP_LIVE_AND_PACK_ACTIONABLE_RUN_RC:-0}"
EOF_FAKE
chmod +x "$FAKE_SCRIPT"
...
echo "[easy-node-roadmap-live-and-pack-actionable] help contract"
./scripts/easy_node.sh help >"$HELP_OUT"
if ! grep -F -- './scripts/easy_node.sh roadmap-live-and-pack-actionable-run' "$HELP_OUT" >/dev/null 2>&1; then
  echo "easy_node help missing roadmap-live-and-pack-actionable-run command contract"
  cat "$HELP_OUT"
  exit 1
fi
for expected in \
  ...
  if ! grep -F -- "$expected" "$HELP_OUT" >/dev/null 2>&1; then
    echo "easy_node help missing roadmap-live-and-pack-actionable-run flag token: $expected"
    cat "$HELP_OUT"
    exit 1
  fi
done
./scripts/easy_node.sh help --expert >"$HELP_EXPERT_OUT"
if ! grep -F -- 'roadmap-live-and-pack-actionable-run wraps the roadmap live+pack actionable helper path' "$HELP_EXPERT_OUT" >/dev/null 2>&1; then
  echo "easy_node expert help missing roadmap-live-and-pack-actionable-run description"
  cat "$HELP_EXPERT_OUT"
  exit 1
fi
if ! grep -F -- 'roadmap_live_and_pack_actionable_run_summary' "$HELP_EXPERT_OUT" >/dev/null 2>&1; then
  echo "easy_node expert help missing roadmap-live-and-pack summary schema token"
  cat "$HELP_EXPERT_OUT"
  exit 1
fi

echo "[easy-node-roadmap-live-and-pack-actionable] env override + forwarding contract"
: >"$CAPTURE"
...
if ! grep -F -- 'fake roadmap live-and-pack actionable run:' "$STDOUT_OUT" >/dev/null 2>&1; then
  echo "missing wrapper output from fake script"
  cat "$STDOUT_OUT"
  exit 1
fi

echo "[easy-node-roadmap-live-and-pack-actionable] output + exit semantics contract"
set +e
ROADMAP_LIVE_AND_PACK_ACTIONABLE_RUN_SCRIPT="$FAKE_SCRIPT" \
ROADMAP_LIVE_AND_PACK_ACTIONABLE_CAPTURE_FILE="$CAPTURE" \
...
echo "[easy-node-roadmap-live-and-pack-actionable] unknown args fail contract"
set +e
ROADMAP_LIVE_AND_PACK_ACTIONABLE_RUN_SCRIPT="$FAKE_SCRIPT" \
ROADMAP_LIVE_AND_PACK_ACTIONABLE_CAPTURE_FILE="$CAPTURE" \
./scripts/easy_node.sh roadmap-live-and-pack-actionable-run --totally-unknown 1 >"$STDOUT_OUT" 2>"$STDERR_OUT"
rc=$?
set -e
if [[ "$rc" -ne 64 ]]; then
...
echo "easy node roadmap live and pack actionable run integration check ok"
