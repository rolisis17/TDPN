...
scripts/integration_three_machine_docker_readiness_public_bind.sh
...
scripts/integration_real_host_ssh_diag.sh
...
Options:
  ...
  --allow-missing-defaults [0|1]
                               Allow missing declared default checks (default: 0)
...
Failure mode:
  ...
  Also fails closed when declared default checks are missing unless
...
validate_summary_invariants() {
  local idx expected_result_count result_state result_code result_path_value result_timed_out_value
  ...
  if [[ "$executed_count" -ne "${#result_path[@]}" || "$executed_count" -ne "${#result_status[@]}" || "$executed_count" -ne "${#result_rc[@]}" || "$executed_count" -ne "${#result_duration[@]}" || "$executed_count" -ne "${#result_log_path[@]}" || "$executed_count" -ne "${#result_timed_out[@]}" ]]; then
  ...
  case "$selection_error" in
    ...
    "no_checks_selected")
      ...
      ;;
    "missing_default_checks")
      if [[ "$missing_default_count" -le 0 ]]; then
        invariant_error="missing_default_checks requires missing_default_checks>0"
        return 1
      fi
      if [[ "$allow_missing_defaults" == "1" ]]; then
        invariant_error="missing_default_checks cannot be set when allow_missing_defaults=1"
        return 1
      fi
      ;;
...
for cmd in bash cat date dirname basename mkdir mktemp mv sleep tail; do
  need_cmd "$cmd"
done
...
allow_missing_defaults="${GPM_LOGIC_CHECK_ALLOW_MISSING_DEFAULTS:-0}"
fail_fast="${GPM_LOGIC_CHECK_FAIL_FAST:-0}"
check_timeout_sec="${GPM_LOGIC_CHECK_CHECK_TIMEOUT_SEC:-300}"
progress_interval_sec="${GPM_LOGIC_CHECK_PROGRESS_INTERVAL_SEC:-60}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --allow-missing-defaults)
      if [[ $# -ge 2 && ( "${2:-}" == "0" || "${2:-}" == "1" ) ]]; then
        allow_missing_defaults="${2:-}"
        shift 2
      else
        allow_missing_defaults="1"
        shift
      fi
      ;;
    --allow-missing-defaults=*)
...
bool_arg_or_die "--allow-missing-defaults" "$allow_missing_defaults"
...
declare -a missing_default_checks=()
...
for rel_path in "${default_checks_rel[@]}"; do
  ...
  if [[ -f "$candidate_path" ]]; then
    ...
    missing_default_checks+=("$rel_path")
  fi
done
...
missing_default_count="${#missing_default_checks[@]}"
...
if [[ "$missing_default_count" -gt 0 && "$allow_missing_defaults" != "1" ]]; then
  selection_error="missing_default_checks"
  overall_rc=1
  echo "[gpm-logic-check] missing declared default checks (fail-closed)" >&2
  for rel_path in "${missing_default_checks[@]}"; do
    echo "[gpm-logic-check] missing_default_check=$rel_path" >&2
  done
  ...
  overall_rc=1
  echo "[gpm-logic-check] invalid selected checks (fail-closed)" >&2
  for invalid_check in "${invalid_selected_checks[@]}"; do
    echo "[gpm-logic-check] invalid_selected_check=$invalid_check" >&2
  done
fi
...
for check_path in "${selected_checks[@]}"; do
  ...
  echo "[gpm-logic-check] check=$check_base status=running timeout_sec=$timeout_display progress_interval_sec=$progress_interval_sec log_path=$log_path"
  start_epoch="$(date +%s)"
  if [[ "$check_timeout_sec" -gt 0 ]]; then
    check_cmd=(timeout --foreground --kill-after=15s "${check_timeout_sec}s" bash "$check_path")
  else
  ...
  if [[ "$progress_interval_sec" -gt 0 ]]; then
    ...
    while kill -0 "$check_pid" >/dev/null 2>&1; do
      ...
      if [[ "$now_epoch" -ge "$next_progress_epoch" ]]; then
        ...
        echo "[gpm-logic-check] check=$check_base status=running elapsed_sec=$elapsed_running_sec log_path=$log_path"
  ...
  echo "[gpm-logic-check] check=$check_base status=$check_status rc=$check_rc duration_sec=$duration_sec timed_out=$check_timed_out"
  if [[ "$check_status" == "fail" ]]; then
    if [[ "$check_timed_out" -eq 1 ]]; then
      echo "[gpm-logic-check] check=$check_base failure=timeout timeout_sec=$check_timeout_sec hint=inspect_log_tail_or_raise_timeout"
    else
      echo "[gpm-logic-check] check=$check_base failure=nonzero_exit"
    fi
    if [[ -s "$log_path" ]]; then
      echo "[gpm-logic-check] check=$check_base failure_log_tail_begin"
      tail -n 40 "$log_path"
      echo "[gpm-logic-check] check=$check_base failure_log_tail_end"
    else
      echo "[gpm-logic-check] check=$check_base failure_log_tail_unavailable reason=empty_log"
    fi
  fi
...
allow_missing_defaults_bool="false"
if [[ "$allow_missing_defaults" == "1" ]]; then
  allow_missing_defaults_bool="true"
fi
...

{
  ...
  echo "    \"allow_missing_defaults\": $allow_missing_defaults_bool,"
  ...
  echo "    ],"
  echo "    \"missing_default_checks\": ["
  if [[ "${#missing_default_checks[@]}" -gt 0 ]]; then
    for ((idx = 0; idx < ${#missing_default_checks[@]}; idx++)); do
      comma=","
      if (( idx == ${#missing_default_checks[@]} - 1 )); then
        comma=""
      fi
      echo "      \"$(json_escape "${missing_default_checks[$idx]}")\"$comma"
    done
  fi
  echo "    ],"
