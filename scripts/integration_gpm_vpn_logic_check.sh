...
if [[ "$SYMLINK_OUTPUT_TESTS_ENABLED" == "1" ]]; then
  echo "[gpm-vpn-logic-check] summary symlink refusal"
  ...
  echo "[gpm-vpn-logic-check] reports-dir symlink refusal"
  ...
  echo "[gpm-vpn-logic-check] log symlink refusal"
  ...
  echo "[gpm-vpn-logic-check] symlink output refusal checks skipped (symlink creation unavailable)"
fi
