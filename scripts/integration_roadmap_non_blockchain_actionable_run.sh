...
for redaction_output_file in "$SUMMARY_REDACTION" "$RUN_LOG_REDACTION" "$REDACTION_ACTION_LOG" "$REDACTION_ROADMAP_SUMMARY" "$REDACTION_ROADMAP_LOG" "$REDACTION_ROADMAP_REPORT"; do
  ...
  for redaction_sentinel in \
    super-secret-token \
    inv-secret-subject \
    nonchain-pass-sentinel \
    nonchain-passwd-sentinel \
    nonchain-api-sentinel \
    nonchain-private-sentinel \
    /tmp/nonchain-private.key \
    /tmp/nonchain-provenance.key \
    "nonchain quoted sentinel" \
