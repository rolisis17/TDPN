...
Notes:
  ...
  - access-recovery-real-helper-evidence-run runs the real public helper HTTPS evidence flow in one operator command: signed pilot evidence bundle, trusted verifier receipt, and roadmap refresh. Use --roadmap-refresh 0 for diagnostics/verifier-only runs; those stop at verifier_ready, while the wrapper handoff_complete/status roll-up requires roadmap refresh. The trusted verifier receipt remains the handoff authority (override with `ACCESS_RECOVERY_REAL_HELPER_EVIDENCE_RUN_SCRIPT`).
