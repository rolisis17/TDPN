...
Usage:
  ...
  ./scripts/easy_node.sh roadmap-next-actions-run [--max-actions N] [--action-timeout-sec N] [--parallel [0|1]] [--allow-profile-default-gate-unreachable [0|1]] [--profile-default-gate-subject ID] [--access-recovery-trust-store PATH] [--include-id ID] [--exclude-id ID] [--include-id-prefix PREFIX] [--exclude-id-prefix PREFIX] [--include-id-suffix SUFFIX] [--exclude-id-suffix SUFFIX] [roadmap_next_actions_run args...]
...
Notes:
  ...
  - roadmap-next-actions-run wraps roadmap `next_actions` in one command with optional exact-id, id-prefix, and id-suffix filtering, per-action timeout, parallel execution controls, optional soft-fail for unreachable profile-default endpoints, explicit profile-default subject override via `--profile-default-gate-subject ID`, and Access Recovery trusted verifier trust-store override via `--access-recovery-trust-store PATH`.
