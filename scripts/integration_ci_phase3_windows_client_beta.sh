...
GATE_SCRIPT="$ROOT_DIR/scripts/ci_phase3_windows_client_beta.sh"
...
STAGE_ENV_NAMES=(
  ...
  "CI_PHASE3_WINDOWS_CLIENT_BETA_EASY_MODE_LAUNCHER_WIRING_SCRIPT"
  "CI_PHASE3_WINDOWS_CLIENT_BETA_EASY_MODE_LAUNCHER_RUNTIME_SCRIPT"
  "CI_PHASE3_WINDOWS_CLIENT_BETA_PHASE3_WINDOWS_CLIENT_BETA_CHECK_SCRIPT"
  "CI_PHASE3_WINDOWS_CLIENT_BETA_PHASE3_WINDOWS_CLIENT_BETA_RUN_SCRIPT"
  "CI_PHASE3_WINDOWS_CLIENT_BETA_PHASE3_WINDOWS_CLIENT_BETA_HANDOFF_CHECK_SCRIPT"
  "CI_PHASE3_WINDOWS_CLIENT_BETA_PHASE3_WINDOWS_CLIENT_BETA_HANDOFF_RUN_SCRIPT"
)
...
capture="${CI_PHASE3_CAPTURE_FILE:?}"
stage_id="${CI_PHASE3_STAGE_ID:?}"
...
fail_matrix="${CI_PHASE3_FAIL_MATRIX:-}"
...
#!/usr/bin/env bash
set -euo pipefail
CI_PHASE3_CAPTURE_FILE="\${CI_PHASE3_CAPTURE_FILE:?}" \
CI_PHASE3_STAGE_ID="$stage_id" \
CI_PHASE3_FAIL_MATRIX="\${CI_PHASE3_FAIL_MATRIX:-}" \
"$FAKE_STAGE_HELPER" "\$@"
EOF_FAKE_STAGE
  chmod +x "$fake_stage"
...
echo "[ci-phase3-windows-client-beta] success ordering path"
: >"$CAPTURE"
...
if ! jq -e '
  .status == "pass"
  and .rc == 0
  and .schema.id == "ci_phase3_windows_client_beta_summary"
  and .schema.major == 1
  and .schema.minor == 0
  and .inputs.dry_run == false
  and .inputs.run_phase3_windows_client_beta_check == true
  ...
  and .steps.phase3_windows_client_beta_handoff_run.rc == 0
' "$SUCCESS_SUMMARY_JSON" >/dev/null; then
  echo "success summary missing expected contract fields"
  cat "$SUCCESS_SUMMARY_JSON"
  exit 1
fi
if ! grep -Fq -- '[ci-phase3-windows-client-beta] status=pass rc=0 dry_run=0' "$SUCCESS_LOG"; then
  echo "success log missing final pass status line"
  cat "$SUCCESS_LOG"
  exit 1
fi

echo "[ci-phase3-windows-client-beta] dry-run skip accounting"
: >"$CAPTURE"
CI_PHASE3_CAPTURE_FILE="$CAPTURE" \
"$GATE_SCRIPT" \
  --dry-run 1 \
...
if ! jq -e '
  ...
  and .steps.phase3_windows_client_beta_run.reason == "dry-run"
  and .steps.phase3_windows_client_beta_handoff_check.status == "skip"
  and .steps.phase3_windows_client_beta_handoff_check.reason == "dry-run"
  and .steps.phase3_windows_client_beta_handoff_run.status == "skip"
  and .steps.phase3_windows_client_beta_handoff_run.reason == "dry-run"
...
if ! grep -Fq -- '[ci-phase3-windows-client-beta] status=pass rc=0 dry_run=1' "$DRY_RUN_LOG"; then
  echo "dry-run log missing final pass status line"
  cat "$DRY_RUN_LOG"
  exit 1
fi
if ! grep -Fq -- 'step=desktop_scaffold_contract status=skip reason=dry-run' "$DRY_RUN_LOG"; then
  echo "dry-run log missing desktop_scaffold_contract skip signal"
  cat "$DRY_RUN_LOG"
  exit 1
fi
...
"$GATE_SCRIPT" \
  ...
  --run-phase3-windows-client-beta-run 0 \
  --run-phase3-windows-client-beta-handoff-check 0 \
  --run-phase3-windows-client-beta-handoff-run 0 >"$TOGGLE_LOG" 2>&1
...
if ! jq -e '
  .status == "pass"
  and .rc == 0
  and .inputs.run_desktop_scaffold_contract == false
  ...
  and .inputs.run_phase3_windows_client_beta_run == false
  and .inputs.run_phase3_windows_client_beta_handoff_check == false
  and .inputs.run_phase3_windows_client_beta_handoff_run == false
  and .steps.phase3_windows_client_beta_check.enabled == false
  and .steps.phase3_windows_client_beta_check.status == "skip"
  and .steps.phase3_windows_client_beta_check.reason == "disabled"
  and .steps.phase3_windows_client_beta_run.enabled == false
  and .steps.phase3_windows_client_beta_run.status == "skip"
  and .steps.phase3_windows_client_beta_run.reason == "disabled"
  and .steps.phase3_windows_client_beta_handoff_check.enabled == false
  and .steps.phase3_windows_client_beta_handoff_check.status == "skip"
  and .steps.phase3_windows_client_beta_handoff_check.reason == "disabled"
  and .steps.phase3_windows_client_beta_handoff_run.enabled == false
  and .steps.phase3_windows_client_beta_handoff_run.status == "skip"
  and .steps.phase3_windows_client_beta_handoff_run.reason == "disabled"
...
echo "[ci-phase3-windows-client-beta] first-failure rc propagation"
: >"$CAPTURE"
set +e
CI_PHASE3_CAPTURE_FILE="$CAPTURE" \
...
if ! jq -e '
  ...
  and .steps.easy_node_config_v1.rc == 41
  and .steps.phase3_windows_client_beta_check.status == "fail"
  and .steps.phase3_windows_client_beta_check.rc == 47
  and .steps.phase3_windows_client_beta_run.status == "fail"
  and .steps.phase3_windows_client_beta_run.rc == 53
  and .steps.phase3_windows_client_beta_handoff_check.status == "fail"
  and .steps.phase3_windows_client_beta_handoff_check.rc == 55
  and .steps.phase3_windows_client_beta_handoff_run.status == "fail"
  and .steps.phase3_windows_client_beta_handoff_run.rc == 59
  and .steps.easy_mode_launcher_runtime.status == "pass"
...
echo "ci phase3 windows client beta integration check ok"
