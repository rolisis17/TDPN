...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --run-settlement-shadow-status-surface)
      ...
      ;;
    --run-issuer-sponsor-api-live-smoke)
      if [[ $# -ge 2 && ( "${2:-}" == "0" || "${2:-}" == "1" ) ]]; then
        run_issuer_sponsor_api_live_smoke="${2:-}"
        shift 2
      else
        run_issuer_sponsor_api_live_smoke="1"
        shift
      fi
      ;;
    --run-issuer-admin-blockchain-handlers-coverage)
      if [[ $# -ge 2 && ( "${2:-}" == "0" || "${2:-}" == "1" ) ]]; then
        ...
        run_issuer_admin_blockchain_handlers_coverage="1"
        shift
      fi
      ;;
