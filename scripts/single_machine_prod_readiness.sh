...
profile_default_gate_stability_cycle_summary_available="0"
profile_default_gate_stability_cycle_decision=""
profile_default_gate_stability_cycle_status=""
...
if [[ -f "$profile_default_gate_stability_cycle_summary_json" ]] && jq -e . "$profile_default_gate_stability_cycle_summary_json" >/dev/null 2>&1; then
  if jq -e '. | type == "object" and (.schema | type) == "object" and (.schema.id // "") == "profile_default_gate_stability_cycle_summary" and (.version == 1) and (.decision | type) == "string" and (.status | type) == "string" and (.rc | type) == "number"' "$profile_default_gate_stability_cycle_summary_json" >/dev/null 2>&1; then
    profile_default_gate_stability_cycle_summary_available="1"
    profile_default_gate_stability_cycle_decision="$(jq -r 'if (.decision | type) == "string" then .decision else "" end' "$profile_default_gate_stability_cycle_summary_json")"
    profile_default_gate_stability_cycle_status="$(jq -r 'if (.status | type) == "string" then .status else "" end' "$profile_default_gate_stability_cycle_summary_json")"
    profile_default_gate_stability_cycle_rc="$(jq -r 'if (.rc | type) == "number" then .rc else "" end' "$profile_default_gate_stability_cycle_summary_json")"
    profile_default_gate_stability_cycle_failure_stage="$(jq -r 'if (.failure_stage | type) == "string" then .failure_stage else "" end' "$profile_default_gate_stability_cycle_summary_json")"
...
summary_payload="$({
  jq -n \
    ...
    --arg profile_default_gate_stability_cycle_summary_available "$profile_default_gate_stability_cycle_summary_available" \
      ...
      summary: {
        ...
        profile_default_gate: (
          (if ($profile_default_gate | type) == "object" then $profile_default_gate else {} end) + {
            ...
            cycle_summary_available: ($profile_default_gate_stability_cycle_summary_available == "1"),
...
if [[ "$profile_default_gate_stability_cycle_summary_available" == "1" ]]; then
  echo "[single-machine-prod-readiness] profile_default_gate_stability_cycle_available=true"
