...
usage() {
  cat <<'USAGE'
Usage:
  ./scripts/phase6_cosmos_l1_build_testnet_run.sh \
    [--reports-dir DIR] \
...
Purpose:
  One-command Phase-6 Cosmos L1 build/testnet runner:
    1) ci_phase6_cosmos_l1_build_testnet.sh
    2) phase6_cosmos_l1_build_testnet_check.sh

Notes:
  - Wrapper-owned flags are reserved; stage pass-through uses prefixes:
      --ci-...     -> forwarded to ci_phase6_cosmos_l1_build_testnet.sh
      --check-...  -> forwarded to phase6_cosmos_l1_build_testnet_check.sh
  - Dry-run forwards --dry-run 1 to ci_phase6 only.
...
run_step_capture() {
  ...
  shift 2
  local rc=0
  echo "[phase6-cosmos-l1-run] step=$label status=running"
  set +e
  "$@" >"$log_path" 2>&1
  rc=$?
  if (( rc == 0 )); then
    echo "[phase6-cosmos-l1-run] step=$label status=pass rc=0"
  else
    echo "[phase6-cosmos-l1-run] step=$label status=fail rc=$rc"
  fi
  return "$rc"
}
...
reports_dir="${PHASE6_COSMOS_L1_BUILD_TESTNET_RUN_REPORTS_DIR:-$ROOT_DIR/.easy-node-logs}"
ci_summary_json="${PHASE6_COSMOS_L1_BUILD_TESTNET_RUN_CI_SUMMARY_JSON:-$reports_dir/phase6_cosmos_l1_build_testnet_ci_summary.json}"
check_summary_json="${PHASE6_COSMOS_L1_BUILD_TESTNET_RUN_CHECK_SUMMARY_JSON:-$reports_dir/phase6_cosmos_l1_build_testnet_check_summary.json}"
summary_json="${PHASE6_COSMOS_L1_BUILD_TESTNET_RUN_SUMMARY_JSON:-$reports_dir/phase6_cosmos_l1_build_testnet_run_summary.json}"
canonical_summary_json="${PHASE6_COSMOS_L1_BUILD_TESTNET_RUN_CANONICAL_SUMMARY_JSON:-$ROOT_DIR/.easy-node-logs/phase6_cosmos_l1_build_testnet_run_summary.json}"
print_summary_json="${PHASE6_COSMOS_L1_BUILD_TESTNET_RUN_PRINT_SUMMARY_JSON:-1}"
dry_run="${PHASE6_COSMOS_L1_BUILD_TESTNET_RUN_DRY_RUN:-0}"
...
ci_script="${PHASE6_COSMOS_L1_BUILD_TESTNET_RUN_CI_SCRIPT:-$ROOT_DIR/scripts/ci_phase6_cosmos_l1_build_testnet.sh}"
check_script="${PHASE6_COSMOS_L1_BUILD_TESTNET_RUN_CHECK_SCRIPT:-$ROOT_DIR/scripts/phase6_cosmos_l1_build_testnet_check.sh}"
...
set +e
run_step_capture "ci_phase6" "$ci_log" "${ci_command_args[@]}"
ci_command_rc=$?
set -e
...
check_command_args=("$check_script" --ci-phase6-summary-json "$ci_summary_json" --summary-json "$check_summary_json")
...
if [[ "$dry_run" == "1" ]]; then
  if ! array_has_arg "--require-chain-scaffold-ok" "${check_command_args[@]:1}"; then
    check_command_args+=(--require-chain-scaffold-ok 0)
  fi
  if ! array_has_arg "--require-proto-surface-ok" "${check_command_args[@]:1}"; then
    check_command_args+=(--require-proto-surface-ok 0)
  fi
  if ! array_has_arg "--require-proto-codegen-surface-ok" "${check_command_args[@]:1}"; then
    check_command_args+=(--require-proto-codegen-surface-ok 0)
  fi
  ...
  if ! array_has_arg "--require-tdpnd-grpc-runtime-smoke-ok" "${check_command_args[@]:1}"; then
    check_command_args+=(--require-tdpnd-grpc-runtime-smoke-ok 0)
  fi
  if ! array_has_arg "--require-tdpnd-grpc-live-smoke-ok" "${check_command_args[@]:1}"; then
    check_command_args+=(--require-tdpnd-grpc-live-smoke-ok 0)
  fi
...
set +e
run_step_capture "phase6_cosmos_l1_build_testnet_check" "$check_log" "${check_command_args[@]}"
check_command_rc=$?
set -e
if check_summary_contract_valid "$check_summary_json"; then
  ...
  check_contract_error="missing or invalid phase6 cosmos l1 build testnet check summary contract"
  check_status="fail"
  if [[ "$check_command_rc" -ne 0 ]]; then
    check_rc="$check_command_rc"
  else
...
jq -n \
  ...
  '{
    ...
    metadata: {
      ...
      runner_script: "phase6_cosmos_l1_build_testnet_run.sh"
    },
    inputs: {
...
echo "[phase6-cosmos-l1-run] status=$final_status rc=$final_rc dry_run=$dry_run"
echo "[phase6-cosmos-l1-run] reports_dir=$reports_dir"
echo "[phase6-cosmos-l1-run] summary_json=$summary_json"
echo "[phase6-cosmos-l1-run] canonical_summary_json=$canonical_summary_json"
