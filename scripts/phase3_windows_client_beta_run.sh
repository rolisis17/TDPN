...
Purpose:
  One-command Phase-3 Windows client-beta runner:
    1) ci_phase3_windows_client_beta.sh
    2) phase3_windows_client_beta_check.sh

Notes:
  - Wrapper-owned flags are reserved; stage pass-through uses prefixes:
      --ci-...     -> forwarded to ci_phase3_windows_client_beta.sh
      --check-...  -> forwarded to phase3_windows_client_beta_check.sh
  - Dry-run forwards --dry-run 1 to ci_phase3 only.
...
run_step_capture() {
  ...
  shift 2
  local rc=0
  echo "[phase3-windows-client-beta-run] step=$label status=running"
  set +e
  "$@" >"$log_path" 2>&1
  rc=$?
  if (( rc == 0 )); then
    echo "[phase3-windows-client-beta-run] step=$label status=pass rc=0"
  else
    echo "[phase3-windows-client-beta-run] step=$label status=fail rc=$rc"
  fi
  return "$rc"
}
...
reports_dir="${PHASE3_WINDOWS_CLIENT_BETA_RUN_REPORTS_DIR:-$ROOT_DIR/.easy-node-logs}"
ci_summary_json="${PHASE3_WINDOWS_CLIENT_BETA_RUN_CI_SUMMARY_JSON:-$reports_dir/phase3_windows_client_beta_ci_summary.json}"
check_summary_json="${PHASE3_WINDOWS_CLIENT_BETA_RUN_CHECK_SUMMARY_JSON:-$reports_dir/phase3_windows_client_beta_check_summary.json}"
summary_json="${PHASE3_WINDOWS_CLIENT_BETA_RUN_SUMMARY_JSON:-$reports_dir/phase3_windows_client_beta_run_summary.json}"
print_summary_json="${PHASE3_WINDOWS_CLIENT_BETA_RUN_PRINT_SUMMARY_JSON:-1}"
dry_run="${PHASE3_WINDOWS_CLIENT_BETA_RUN_DRY_RUN:-0}"
...
ci_script="${PHASE3_WINDOWS_CLIENT_BETA_RUN_CI_SCRIPT:-$ROOT_DIR/scripts/ci_phase3_windows_client_beta.sh}"
check_script="${PHASE3_WINDOWS_CLIENT_BETA_RUN_CHECK_SCRIPT:-$ROOT_DIR/scripts/phase3_windows_client_beta_check.sh}"
...
set +e
run_step_capture "ci_phase3" "$ci_log" "${ci_command_args[@]}"
ci_command_rc=$?
set -e
if ci_summary_contract_valid "$ci_summary_json"; then
  ...
  ci_contract_valid=0
  ci_contract_error="missing or invalid ci_phase3 summary contract"
  ci_status="fail"
  if [[ "$ci_command_rc" -ne 0 ]]; then
    ci_rc="$ci_command_rc"
  else
...
check_command_args=("$check_script" --ci-phase3-summary-json "$ci_summary_json" --summary-json "$check_summary_json")
...
if [[ "$dry_run" == "1" ]]; then
  if ! array_has_arg "--require-desktop-scaffold-ok" "${check_command_args[@]:1}"; then
    check_command_args+=(--require-desktop-scaffold-ok 0)
  fi
  if ! array_has_arg "--require-local-control-api-ok" "${check_command_args[@]:1}"; then
    check_command_args+=(--require-local-control-api-ok 0)
  fi
  if ! array_has_arg "--require-local-api-config-defaults-ok" "${check_command_args[@]:1}"; then
    check_command_args+=(--require-local-api-config-defaults-ok 0)
  fi
  if ! array_has_arg "--require-easy-node-config-v1-ok" "${check_command_args[@]:1}"; then
    check_command_args+=(--require-easy-node-config-v1-ok 0)
  fi
  if ! array_has_arg "--require-launcher-wiring-ok" "${check_command_args[@]:1}"; then
    check_command_args+=(--require-launcher-wiring-ok 0)
  fi
...
set +e
run_step_capture "phase3_windows_client_beta_check" "$check_log" "${check_command_args[@]}"
check_command_rc=$?
set -e
if check_summary_contract_valid "$check_summary_json"; then
  ...
  check_failure_kind="$(jq -r '.failure.kind // ""' "$check_summary_json" 2>/dev/null || true)"
  ...
  check_contract_error="missing or invalid phase3 Windows client-beta check summary contract"
  check_status="fail"
  if [[ "$check_command_rc" -ne 0 ]]; then
    check_rc="$check_command_rc"
  else
...
jq -n \
  ...
  --arg check_failure_kind "$check_failure_kind" \
  ...
  '{
    ...
    metadata: {
      ...
      runner_script: "phase3_windows_client_beta_run.sh"
    },
    inputs: {
    ...
    steps: {
      ...
      phase3_windows_client_beta_check: {
        ...
        failure: {
          kind: (if $check_failure_kind == "" then null else $check_failure_kind end),
          policy_no_go: ($check_failure_kind == "policy_no_go"),
          execution_failure: ($check_failure_kind == "execution_failure"),
          timeout: ($check_failure_kind == "timeout")
        },
        policy_outcome: {
          decision: (if $check_policy_outcome_decision == "" then null else $check_policy_outcome_decision end),
          fail_closed_no_go: ($check_failure_kind == "policy_no_go")
        },
        actionable: {
