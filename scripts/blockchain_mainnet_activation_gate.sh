...
Notes:
  ...
  - Use --fail-close=1 to return non-zero when the decision is NO-GO or when
    the metrics input is missing/invalid.
  - Exit codes:
      0: help or evaluation succeeded and fail-close is off
...
fail_close="${BLOCKCHAIN_MAINNET_ACTIVATION_GATE_FAIL_CLOSE:-0}"
...
echo "[blockchain-mainnet-activation-gate] decision=$decision status=$status rc=$summary_rc exit_code=$exit_code fail_close=$fail_close metrics_json=$metrics_json summary_json=$summary_json"
