...
Purpose:
  Evaluate the bootstrap metrics gate for blockchain mainnet activation and
  emit one GO/NO-GO summary JSON.

Notes:
  - The metrics input path can be provided by:
    - --metrics-json PATH
    - BLOCKCHAIN_MAINNET_ACTIVATION_GATE_METRICS_JSON
  - The summary path can be provided by:
    - --summary-json PATH
    - BLOCKCHAIN_MAINNET_ACTIVATION_GATE_SUMMARY_JSON
  - Default mode is enforce: real evidence is required and NO-GO/missing/
    invalid input returns non-zero. Use --enforce-launch to make that posture
    explicit in launch automation.
  - Use --report-only for diagnostic/report generation that must emit a
    NO-GO summary without treating the shell exit code as approval.
  - Enforce mode rejects ad hoc metric JSON and requires a metrics-summary
    schema with production source-json provenance for every required gate
    metric. Each usable source JSON must carry evidence.mode=production,
    an ISO-UTC evidence.generated_at, and an allowlisted production
    evidence.source_kind.
    Enforce mode also rejects production evidence that is stale or generated
    in the future. The freshness window defaults to 1209600 seconds (14 days)
    and can be overridden with --evidence-max-age-sec or
    BLOCKCHAIN_MAINNET_ACTIVATION_GATE_EVIDENCE_MAX_AGE_SEC.
  - Exit codes:
      0: help, GO in enforce mode, or report-only evaluation completed
      1: fail-close triggered on NO-GO or invalid/missing input
...
validate_real_evidence_contract() {
  ...
  if ! jq -e '
    ...
    . as $root
    | ($root.schema.id == "blockchain_mainnet_activation_metrics_summary")
    ...
    and (all(required_metrics[];
	  ...
	  ' "$metrics_path" >/dev/null 2>&1; then
	    printf '%s' "metrics JSON lacks required real-evidence provenance: expected blockchain_mainnet_activation_metrics_summary with complete source-json evidence bindings for every required metric"
	    return 0
	  fi
...
evaluate_compound_gate() {
  ...
  local reason=""
  local actual_json="$7"
  local threshold_json="$8"

  local gate_json
  gate_json="$(make_gate_json "$gate_id" "$title" "$category" "$metric" "$comparison" "$unit" "$status" "$reason" "$actual_json" "$threshold_json")"
  printf '%s' "$gate_json"
}

metrics_json="${BLOCKCHAIN_MAINNET_ACTIVATION_GATE_METRICS_JSON:-}"
summary_json="${BLOCKCHAIN_MAINNET_ACTIVATION_GATE_SUMMARY_JSON:-}"
print_summary_json="${BLOCKCHAIN_MAINNET_ACTIVATION_GATE_PRINT_SUMMARY_JSON:-0}"
gate_mode="${BLOCKCHAIN_MAINNET_ACTIVATION_GATE_MODE:-enforce}"
fail_close="${BLOCKCHAIN_MAINNET_ACTIVATION_GATE_FAIL_CLOSE:-}"
require_real_evidence="${BLOCKCHAIN_MAINNET_ACTIVATION_GATE_REQUIRE_REAL_EVIDENCE:-}"
evidence_max_age_sec="${BLOCKCHAIN_MAINNET_ACTIVATION_GATE_EVIDENCE_MAX_AGE_SEC:-1209600}"
...
if [[ "$input_valid" != "1" ]]; then
  ...
  jq -n \
    --arg schema_id "blockchain_mainnet_activation_gate_summary" \
  ...
  gate_json_entries+=("$(evaluate_single_gate "$metrics_json" "paid_sessions_per_day_30d_avg" "Demand - Sustained traffic" "Demand" "paid_sessions_per_day_30d_avg" ">=" "10000" "sessions/day")")
  ...
  elif ! metric_value_is_valid "validator_independent_operators" "$validator_independent_operators" || ! metric_value_is_valid "validator_max_operator_seat_share_pct" "$validator_max_operator_seat_share_pct"; then
    validator_operator_reason="missing or invalid metric: validator_independent_operators or validator_max_operator_seat_share_pct"
  ...
  validator_max_asn_provider_seat_share_pct="$(json_number_or_empty "$metrics_json" '.validator_max_asn_provider_seat_share_pct')"
  validator_infra_status="fail"
  validator_infra_reason=""
  validator_infra_actual_json="null"
  validator_infra_threshold_json='{"max_asn_provider_seat_share_pct":25}'
  if [[ -z "$validator_max_asn_provider_seat_share_pct" ]]; then
    validator_infra_reason="missing or invalid metric: validator_max_asn_provider_seat_share_pct"
  elif ! metric_value_is_valid "validator_max_asn_provider_seat_share_pct" "$validator_max_asn_provider_seat_share_pct"; then
    validator_infra_reason="missing or invalid metric: validator_max_asn_provider_seat_share_pct"
    validator_infra_actual_json="$(jq -n --argjson max_asn_provider_seat_share_pct "$validator_max_asn_provider_seat_share_pct" '{max_asn_provider_seat_share_pct: $max_asn_provider_seat_share_pct}')"
  elif numeric_compare_ok "$validator_max_asn_provider_seat_share_pct" "<=" "25"; then
    validator_infra_status="pass"
    validator_infra_reason=""
    validator_infra_actual_json="$(jq -n --argjson max_asn_provider_seat_share_pct "$validator_max_asn_provider_seat_share_pct" '{max_asn_provider_seat_share_pct: $max_asn_provider_seat_share_pct}')"
  else
    validator_infra_reason="max_asn_provider_seat_share_pct=$validator_max_asn_provider_seat_share_pct must be <= 25"
    validator_infra_actual_json="$(jq -n --argjson max_asn_provider_seat_share_pct "$validator_max_asn_provider_seat_share_pct" '{max_asn_provider_seat_share_pct: $max_asn_provider_seat_share_pct}')"
  fi
  gate_json_entries+=("$(make_gate_json "validator_infra_concentration" "Validator decentralization - Infra concentration" "Validator decentralization" "validator_max_asn_provider_seat_share_pct" "<=" "percent" "$validator_infra_status" "$validator_infra_reason" "$validator_infra_actual_json" "$validator_infra_threshold_json")")
  ...
  elif ! metric_value_is_valid "validator_region_count" "$validator_region_count" || ! metric_value_is_valid "validator_country_count" "$validator_country_count"; then
    validator_geo_reason="missing or invalid metric: validator_region_count or validator_country_count"
  ...
  gate_json_entries+=("$(evaluate_single_gate "$metrics_json" "subsidy_sustainability" "Economics - Subsidy sustainability" "Economics" "subsidy_runway_months" ">=" "12" "months")")
  gate_json_entries+=("$(evaluate_single_gate "$metrics_json" "unit_economics" "Economics - Unit economics" "Economics" "contribution_margin_3mo" ">" "0" "margin")")
...
echo "[blockchain-mainnet-activation-gate] decision=$decision status=$status rc=$summary_rc exit_code=$exit_code mode=$gate_mode fail_close=$fail_close require_real_evidence=$require_real_evidence metrics_json=$metrics_json summary_json=$summary_json"
echo "[blockchain-mainnet-activation-gate] failed_gate_ids=$(jq -r '.failed_gate_ids | if length == 0 then "none" else join(",") end' "$summary_json")"
echo "[blockchain-mainnet-activation-gate] failed_reasons=$(jq -r '.failed_reasons | if length == 0 then "none" else join(" | ") end' "$summary_json")"
