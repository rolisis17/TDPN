...
usage() {
  cat <<'USAGE'
Usage:
  ./scripts/roadmap_blockchain_actionable_run.sh \
    [--reports-dir DIR] \
    [--summary-json PATH] \
...
Purpose:
  Resolve blockchain actionable commands from roadmap_progress_report next_actions
  (ids starting with blockchain_), then execute those commands in one wrapper run.
...
run_action_command_string() {
  ...
  local pre_exec_revalidate_delay_sec="${ROADMAP_BLOCKCHAIN_ACTIONABLE_RUN_PRE_EXEC_REVALIDATE_DELAY_SEC:-0}"
  local pre_exec_revalidate_ready_file="${ROADMAP_BLOCKCHAIN_ACTIONABLE_RUN_PRE_EXEC_REVALIDATE_READY_FILE:-}"
  ...
  if ! command_requires_shell_execution "$command_text" && command_string_to_argv "$command_text"; then
    ...
    if [[ "${#command_argv[@]}" -gt 0 ]]; then
      ...
      if [[ -n "$pre_exec_revalidate_ready_file" ]]; then
        # Integration-only synchronization hook for deterministic TOCTOU regression coverage.
        printf '%s\n' "$validated_script_path_initial" >"$pre_exec_revalidate_ready_file" 2>/dev/null || true
      fi
...
reports_dir="${ROADMAP_BLOCKCHAIN_ACTIONABLE_RUN_REPORTS_DIR:-}"
summary_json="${ROADMAP_BLOCKCHAIN_ACTIONABLE_RUN_SUMMARY_JSON:-}"
roadmap_summary_json="${ROADMAP_BLOCKCHAIN_ACTIONABLE_RUN_ROADMAP_SUMMARY_JSON:-}"
roadmap_report_md="${ROADMAP_BLOCKCHAIN_ACTIONABLE_RUN_ROADMAP_REPORT_MD:-}"
refresh_manual_validation="${ROADMAP_BLOCKCHAIN_ACTIONABLE_RUN_REFRESH_MANUAL_VALIDATION:-0}"
...
allow_unsafe_shell_commands="${ROADMAP_BLOCKCHAIN_ACTIONABLE_RUN_ALLOW_UNSAFE_SHELL_COMMANDS:-0}"
...
if [[ -z "$reports_dir" ]]; then
  reports_dir="$ROOT_DIR/.easy-node-logs/roadmap_blockchain_actionable_run_${run_stamp}"
fi
...
if [[ -z "$summary_json" ]]; then
  summary_json="$reports_dir/roadmap_blockchain_actionable_run_summary.json"
fi
...
roadmap_script="${ROADMAP_BLOCKCHAIN_ACTIONABLE_RUN_ROADMAP_SCRIPT:-$ROOT_DIR/scripts/roadmap_progress_report.sh}"
...
if [[ "$roadmap_paths_provided" != "1" ]]; then
  ...
  echo "[roadmap-blockchain-actionable-run] stage=roadmap_progress_report status=running"
  set +e
  "${roadmap_cmd[@]}" >"$roadmap_log" 2>&1
  roadmap_rc=$?
  set -e
  if (( roadmap_rc != 0 )); then
    echo "[roadmap-blockchain-actionable-run] stage=roadmap_progress_report status=fail rc=$roadmap_rc"
...
for idx in $(seq 0 $(( actions_count - 1 )) 2>/dev/null || true); do
  ...
  if [[ "$refresh_evidence_fail_closed" == "1" && "$action_id" == "$refresh_evidence_action_id" ]]; then
    ...
    echo "[roadmap-blockchain-actionable-run] action=$action_id status=running"
    if [[ "$parallel" == "1" ]]; then
      (
        action_status="fail"
        action_rc=125
...
for idx in $(seq 0 $(( actions_count - 1 )) 2>/dev/null || true); do
  ...
  if [[ "$action_status" == "pass" ]]; then
    echo "[roadmap-blockchain-actionable-run] action=$action_id status=pass rc=0"
  else
    echo "[roadmap-blockchain-actionable-run] action=$action_id status=fail rc=$action_rc failure_kind=$action_failure_kind"
    if [[ -n "$action_notes" ]]; then
      echo "[roadmap-blockchain-actionable-run] action=$action_id notes=$action_notes"
    fi
    echo "[roadmap-blockchain-actionable-run] action=$action_id log=$action_log"
  fi
...
jq -n \
  ...
  '{
    version: 1,
    schema: { id: "roadmap_blockchain_actionable_run_summary", major: 1, minor: 0 },
...
echo "[roadmap-blockchain-actionable-run] status=$final_status rc=$final_rc selected=$actions_count executed=$executed_count pass=$pass_count fail=$fail_count"
echo "[roadmap-blockchain-actionable-run] summary_json=$summary_json"
