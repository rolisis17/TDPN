...
usage() {
  cat <<'USAGE'
Usage:
  ./scripts/phase6_cosmos_l1_build_testnet_handoff_run.sh \
    [--reports-dir DIR] \
...
Purpose:
  One-command Phase-6 Cosmos L1 build/testnet handoff runner:
    1) phase6_cosmos_l1_build_testnet_run.sh
    2) phase6_cosmos_l1_build_testnet_handoff_check.sh

Notes:
  - Wrapper-owned flags are reserved; stage pass-through uses prefixes:
      --run-...      -> forwarded to phase6_cosmos_l1_build_testnet_run.sh
      --handoff-...  -> forwarded to phase6_cosmos_l1_build_testnet_handoff_check.sh
...
run_stage_capture() {
  ...
  shift 2
  local rc=0
  echo "[phase6-cosmos-l1-build-testnet-handoff-run] stage=$label status=running"
  set +e
  "$@" >"$log_path" 2>&1
  rc=$?
  if (( rc == 0 )); then
    echo "[phase6-cosmos-l1-build-testnet-handoff-run] stage=$label status=pass rc=0"
  else
    echo "[phase6-cosmos-l1-build-testnet-handoff-run] stage=$label status=fail rc=$rc"
  fi
  return "$rc"
}
...
extract_roadmap_summary_path() {
  ...
  if [[ -f "$run_summary_json" ]] && jq -e . "$run_summary_json" >/dev/null 2>&1; then
    path="$(jq -r '(.steps.phase6_cosmos_l1_build_testnet_check.artifacts.roadmap_summary_json // .artifacts.roadmap_summary_json // empty)' "$run_summary_json" 2>/dev/null || true)"
  fi
  if [[ -n "$path" ]]; then
...
run_phase6_cosmos_l1_build_testnet_handoff_check="${PHASE6_COSMOS_L1_BUILD_TESTNET_HANDOFF_RUN_RUN_PHASE6_COSMOS_L1_BUILD_TESTNET_HANDOFF_CHECK:-1}"
...
run_script="${PHASE6_COSMOS_L1_BUILD_TESTNET_HANDOFF_RUN_RUN_SCRIPT:-$ROOT_DIR/scripts/phase6_cosmos_l1_build_testnet_run.sh}"
handoff_check_script="${PHASE6_COSMOS_L1_BUILD_TESTNET_HANDOFF_RUN_HANDOFF_CHECK_SCRIPT:-$ROOT_DIR/scripts/phase6_cosmos_l1_build_testnet_handoff_check.sh}"

if [[ "$run_phase6_cosmos_l1_build_testnet_run" == "1" && ! -x "$run_script" ]]; then
  echo "missing executable stage script: $run_script"
  exit 2
fi
if [[ "$run_phase6_cosmos_l1_build_testnet_handoff_check" == "1" && ! -x "$handoff_check_script" ]]; then
...
if [[ -z "$reports_dir" ]]; then
  reports_dir="$ROOT_DIR/.easy-node-logs/phase6_cosmos_l1_build_testnet_handoff_run_${run_stamp}"
else
  reports_dir="$(abs_path "$reports_dir")"
fi
if [[ -z "$run_summary_json" ]]; then
  run_summary_json="$reports_dir/phase6_cosmos_l1_build_testnet_run_summary.json"
else
  run_summary_json="$(abs_path "$run_summary_json")"
fi
if [[ -z "$handoff_summary_json" ]]; then
  handoff_summary_json="$reports_dir/phase6_cosmos_l1_build_testnet_handoff_check_summary.json"
else
  handoff_summary_json="$(abs_path "$handoff_summary_json")"
fi
if [[ -z "$summary_json" ]]; then
  summary_json="$reports_dir/phase6_cosmos_l1_build_testnet_handoff_run_summary.json"
else
...
if [[ "$run_phase6_cosmos_l1_build_testnet_run" == "1" ]]; then
  ...
  echo "[phase6-cosmos-l1-build-testnet-handoff-run] stage=phase6_cosmos_l1_build_testnet_run status=skipped reason=disabled"
fi

declare -a handoff_cmd=(
  "$handoff_check_script"
  --phase6-run-summary-json "$run_summary_json"
...
if [[ "$dry_run" == "1" ]]; then
  ...
  if ! array_has_arg "--require-chain-scaffold-ok" "${handoff_cmd[@]:1}"; then
    handoff_cmd+=(--require-chain-scaffold-ok 0)
  fi
  if ! array_has_arg "--require-proto-surface-ok" "${handoff_cmd[@]:1}"; then
    handoff_cmd+=(--require-proto-surface-ok 0)
  fi
  if ! array_has_arg "--require-proto-codegen-surface-ok" "${handoff_cmd[@]:1}"; then
    handoff_cmd+=(--require-proto-codegen-surface-ok 0)
  fi
  if ! array_has_arg "--require-query-surface-ok" "${handoff_cmd[@]:1}"; then
    handoff_cmd+=(--require-query-surface-ok 0)
  fi
  if ! array_has_arg "--require-grpc-app-roundtrip-ok" "${handoff_cmd[@]:1}"; then
    handoff_cmd+=(--require-grpc-app-roundtrip-ok 0)
  fi
...
if [[ "$run_phase6_cosmos_l1_build_testnet_handoff_check" == "1" ]]; then
  set +e
  run_stage_capture "phase6_cosmos_l1_build_testnet_handoff_check" "$handoff_log" "${handoff_cmd[@]}"
  handoff_command_rc=$?
  set -e
  ...
  echo "[phase6-cosmos-l1-build-testnet-handoff-run] stage=phase6_cosmos_l1_build_testnet_handoff_check status=skipped reason=disabled"
fi
...
if [[ "$run_phase6_cosmos_l1_build_testnet_handoff_check" == "1" ]] && (( handoff_rc != 0 )) && (( final_rc == 0 )); then
  final_rc="$handoff_rc"
fi
...
jq -n \
  ...
  --argjson run_phase6_cosmos_l1_build_testnet_handoff_check "$run_phase6_cosmos_l1_build_testnet_handoff_check" \
  ...
  '{
    ...
    metadata: {
      ...
      runner_script: "phase6_cosmos_l1_build_testnet_handoff_run.sh"
    },
    inputs: {
    ...
    steps: {
      ...
      phase6_cosmos_l1_build_testnet_handoff_check: {
        enabled: ($run_phase6_cosmos_l1_build_testnet_handoff_check == 1),
