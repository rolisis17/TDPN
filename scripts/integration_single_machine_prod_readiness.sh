...
AUTO_REFRESH_FUTURE_GENERATED_AT_LOG="$TMP_DIR/auto_refresh_future_generated_at.log"
...
PROFILE_STABILITY_VALID_SUMMARY="$TMP_DIR/profile_stability_valid_summary.json"
PROFILE_STABILITY_VALID_LOG="$TMP_DIR/profile_stability_valid.log"
PROFILE_STABILITY_INVALID_SUMMARY="$TMP_DIR/profile_stability_invalid_summary.json"
PROFILE_STABILITY_INVALID_LOG="$TMP_DIR/profile_stability_invalid.log"
PROFILE_STABILITY_MISSING_SUMMARY="$TMP_DIR/profile_stability_missing_summary.json"
PROFILE_STABILITY_MISSING_LOG="$TMP_DIR/profile_stability_missing.log"
PROFILE_STABILITY_VALID_SOURCE_JSON="$TMP_DIR/profile_default_gate_stability_valid_source.json"
...
MULTI_VM_STABILITY_CHECK_VALID_SUMMARY="$TMP_DIR/multi_vm_stability_check_valid_summary.json"
MULTI_VM_STABILITY_CHECK_VALID_LOG="$TMP_DIR/multi_vm_stability_check_valid.log"
MULTI_VM_STABILITY_CHECK_INVALID_SUMMARY="$TMP_DIR/multi_vm_stability_check_invalid_summary.json"
MULTI_VM_STABILITY_CHECK_INVALID_LOG="$TMP_DIR/multi_vm_stability_check_invalid.log"
MULTI_VM_STABILITY_CHECK_MISSING_SUMMARY="$TMP_DIR/multi_vm_stability_check_missing_summary.json"
MULTI_VM_STABILITY_CHECK_MISSING_LOG="$TMP_DIR/multi_vm_stability_check_missing.log"
MULTI_VM_STABILITY_CHECK_VALID_SOURCE_JSON="$TMP_DIR/profile_compare_multi_vm_stability_check_valid_source.json"
MULTI_VM_STABILITY_CHECK_INVALID_SOURCE_JSON="$TMP_DIR/profile_compare_multi_vm_stability_check_invalid_source.json"
MULTI_VM_STABILITY_CHECK_MISSING_REPORTS_DIR="$TMP_DIR/profile_compare_multi_vm_stability_check_missing_reports"
MULTI_VM_STABILITY_CHECK_MISSING_DEFAULT_JSON="$MULTI_VM_STABILITY_CHECK_MISSING_REPORTS_DIR/profile_compare_multi_vm_stability_check_summary.json"
PROFILE_STABILITY_CYCLE_VALID_SUMMARY="$TMP_DIR/profile_stability_cycle_valid_summary.json"
PROFILE_STABILITY_CYCLE_VALID_LOG="$TMP_DIR/profile_stability_cycle_valid.log"
PROFILE_STABILITY_CYCLE_INVALID_SUMMARY="$TMP_DIR/profile_stability_cycle_invalid_summary.json"
PROFILE_STABILITY_CYCLE_INVALID_LOG="$TMP_DIR/profile_stability_cycle_invalid.log"
PROFILE_STABILITY_CYCLE_MISSING_SUMMARY="$TMP_DIR/profile_stability_cycle_missing_summary.json"
PROFILE_STABILITY_CYCLE_MISSING_LOG="$TMP_DIR/profile_stability_cycle_missing.log"
PROFILE_STABILITY_CYCLE_VALID_SOURCE_JSON="$TMP_DIR/profile_default_gate_stability_cycle_valid_source.json"
PROFILE_STABILITY_CYCLE_INVALID_SOURCE_JSON="$TMP_DIR/profile_default_gate_stability_cycle_invalid_source.json"
PROFILE_STABILITY_CYCLE_MISSING_REPORTS_DIR="$TMP_DIR/profile_default_gate_stability_cycle_missing_reports"
PROFILE_STABILITY_CYCLE_MISSING_DEFAULT_JSON="$PROFILE_STABILITY_CYCLE_MISSING_REPORTS_DIR/profile_default_gate_stability_cycle_summary.json"
...
if [[ -n "${FAKE_MANUAL_REPORT_STABILITY_SUMMARY_JSON:-}" || -n "${FAKE_MANUAL_REPORT_STABILITY_CHECK_SUMMARY_JSON:-}" || -n "${FAKE_MANUAL_REPORT_STABILITY_CYCLE_SUMMARY_JSON:-}" || -n "${FAKE_MANUAL_REPORT_MULTI_VM_STABILITY_CHECK_SUMMARY_JSON:-}" ]]; then
  ...
  jq \
    ...
    --arg multi_vm_stability_check_summary_json "${FAKE_MANUAL_REPORT_MULTI_VM_STABILITY_CHECK_SUMMARY_JSON:-}" \
    --arg stability_cycle_summary_json "${FAKE_MANUAL_REPORT_STABILITY_CYCLE_SUMMARY_JSON:-}" '
    .summary.profile_default_gate = (
      ...
      + {
          artifacts: (
            ...
            + (if $multi_vm_stability_check_summary_json == "" then {} else {profile_compare_multi_vm_stability_check_summary_json: $multi_vm_stability_check_summary_json} end)
            + (if $stability_cycle_summary_json == "" then {} else {profile_default_gate_stability_cycle_summary_json: $stability_cycle_summary_json} end)
          )
        }
    )
  ' "$summary_json" >"$summary_json_tmp"
  mv -f "$summary_json_tmp" "$summary_json"
fi
printf '# fake report\n' >"$report_md"
...
echo "[single-machine-prod-readiness] profile default gate stability summary (valid)"
cat >"$PROFILE_STABILITY_VALID_SOURCE_JSON" <<'EOF_STABILITY_VALID'
{
  "schema": {
...
EOF_STABILITY_VALID
FAKE_MANUAL_REPORT_MODE=pending_multi \
FAKE_MANUAL_REPORT_STABILITY_SUMMARY_JSON="$PROFILE_STABILITY_VALID_SOURCE_JSON" \
SINGLE_MACHINE_CI_LOCAL_SCRIPT="$FAKE_CI" \
SINGLE_MACHINE_BETA_PREFLIGHT_SCRIPT="$FAKE_BETA" \
SINGLE_MACHINE_DEEP_TEST_SUITE_SCRIPT="$FAKE_DEEP_OK" \
SINGLE_MACHINE_RUNTIME_FIX_RECORD_SCRIPT="$FAKE_RUNTIME_FIX_RECORD" \
...
./scripts/single_machine_prod_readiness.sh \
  ...
  --run-pre-real-host-readiness 0 \
  --run-real-wg-privileged-matrix 0 \
  --profile-compare-campaign-signoff-reports-dir "$TMP_DIR/profile_default_gate_stability_valid_reports" \
  --summary-json "$PROFILE_STABILITY_VALID_SUMMARY" \
  --print-summary-json 0 >"$PROFILE_STABILITY_VALID_LOG"
if ! jq -e --arg stability_json "$PROFILE_STABILITY_VALID_SOURCE_JSON" '
  .summary.profile_default_gate.stability_summary_json == $stability_json
  and .summary.profile_default_gate.stability_summary_available == true
  and .summary.profile_default_gate.stability_status == "pass"
  and .summary.profile_default_gate.stability_rc == 0
  and .summary.profile_default_gate.stability_runs_requested == 4
  and .summary.profile_default_gate.stability_runs_completed == 4
  and .summary.profile_default_gate.stability_selection_policy_present_all == true
  and .summary.profile_default_gate.stability_consistent_selection_policy == true
  and .summary.profile_default_gate.stability_ok == true
  and .summary.profile_default_gate.stability_recommended_profile_counts.balanced == 3
  and .summary.profile_default_gate.stability_recommended_profile_counts.private == 1
' "$PROFILE_STABILITY_VALID_SUMMARY" >/dev/null; then
  echo "valid profile default gate stability summary did not propagate as expected"
  cat "$PROFILE_STABILITY_VALID_LOG"
  cat "$PROFILE_STABILITY_VALID_SUMMARY"
  exit 1
fi
if ! rg -q '\[single-machine-prod-readiness\] profile_default_gate_stability_available=true' "$PROFILE_STABILITY_VALID_LOG"; then
  echo "valid stability path missing availability=true log line"
  cat "$PROFILE_STABILITY_VALID_LOG"
  exit 1
fi
if ! rg -q '\[single-machine-prod-readiness\] profile_default_gate_stability_status=pass rc=0 runs_completed=4/4' "$PROFILE_STABILITY_VALID_LOG"; then
  echo "valid stability path missing status log line"
  cat "$PROFILE_STABILITY_VALID_LOG"
  exit 1
fi

echo "[single-machine-prod-readiness] profile default gate stability summary (invalid schema)"
cat >"$PROFILE_STABILITY_INVALID_SOURCE_JSON" <<'EOF_STABILITY_INVALID'
{
  "schema": {
    "id": "wrong_schema",
    "major": 1,
...
EOF_STABILITY_INVALID
FAKE_MANUAL_REPORT_MODE=pending_multi \
FAKE_MANUAL_REPORT_STABILITY_SUMMARY_JSON="$PROFILE_STABILITY_INVALID_SOURCE_JSON" \
SINGLE_MACHINE_CI_LOCAL_SCRIPT="$FAKE_CI" \
SINGLE_MACHINE_BETA_PREFLIGHT_SCRIPT="$FAKE_BETA" \
SINGLE_MACHINE_DEEP_TEST_SUITE_SCRIPT="$FAKE_DEEP_OK" \
SINGLE_MACHINE_RUNTIME_FIX_RECORD_SCRIPT="$FAKE_RUNTIME_FIX_RECORD" \
...
./scripts/single_machine_prod_readiness.sh \
  ...
  --run-pre-real-host-readiness 0 \
  --run-real-wg-privileged-matrix 0 \
  --profile-compare-campaign-signoff-reports-dir "$TMP_DIR/profile_default_gate_stability_invalid_reports" \
  --summary-json "$PROFILE_STABILITY_INVALID_SUMMARY" \
  --print-summary-json 0 >"$PROFILE_STABILITY_INVALID_LOG"
if ! jq -e --arg stability_json "$PROFILE_STABILITY_INVALID_SOURCE_JSON" '
  .summary.profile_default_gate.stability_summary_json == $stability_json
  and .summary.profile_default_gate.stability_summary_available == false
  and .summary.profile_default_gate.stability_status == null
  and .summary.profile_default_gate.stability_rc == null
  and .summary.profile_default_gate.stability_runs_requested == null
  and .summary.profile_default_gate.stability_runs_completed == null
  and .summary.profile_default_gate.stability_selection_policy_present_all == null
  and .summary.profile_default_gate.stability_consistent_selection_policy == null
  and .summary.profile_default_gate.stability_ok == null
  and .summary.profile_default_gate.stability_recommended_profile_counts == null
' "$PROFILE_STABILITY_INVALID_SUMMARY" >/dev/null; then
  echo "invalid profile default gate stability summary should fail closed"
  cat "$PROFILE_STABILITY_INVALID_LOG"
  cat "$PROFILE_STABILITY_INVALID_SUMMARY"
  exit 1
fi
if ! rg -q '\[single-machine-prod-readiness\] profile_default_gate_stability_available=false' "$PROFILE_STABILITY_INVALID_LOG"; then
  echo "invalid stability path missing availability=false log line"
  cat "$PROFILE_STABILITY_INVALID_LOG"
  exit 1
fi

echo "[single-machine-prod-readiness] profile default gate stability summary (missing default path)"
rm -rf "$PROFILE_STABILITY_MISSING_REPORTS_DIR"
FAKE_MANUAL_REPORT_MODE=pending_multi \
SINGLE_MACHINE_CI_LOCAL_SCRIPT="$FAKE_CI" \
SINGLE_MACHINE_BETA_PREFLIGHT_SCRIPT="$FAKE_BETA" \
SINGLE_MACHINE_DEEP_TEST_SUITE_SCRIPT="$FAKE_DEEP_OK" \
...
./scripts/single_machine_prod_readiness.sh \
  ...
  --run-pre-real-host-readiness 0 \
  --run-real-wg-privileged-matrix 0 \
  --profile-compare-campaign-signoff-reports-dir "$PROFILE_STABILITY_MISSING_REPORTS_DIR" \
  --summary-json "$PROFILE_STABILITY_MISSING_SUMMARY" \
  --print-summary-json 0 >"$PROFILE_STABILITY_MISSING_LOG"
if ! jq -e --arg stability_json "$PROFILE_STABILITY_MISSING_DEFAULT_JSON" '
  .summary.profile_default_gate.stability_summary_json == $stability_json
  and .summary.profile_default_gate.stability_summary_available == false
  and .summary.profile_default_gate.stability_status == null
  and .summary.profile_default_gate.stability_rc == null
  ...
  and .summary.profile_default_gate.stability_ok == null
  and .summary.profile_default_gate.stability_recommended_profile_counts == null
' "$PROFILE_STABILITY_MISSING_SUMMARY" >/dev/null; then
  echo "missing default stability summary should leave additive fields null-safe"
  cat "$PROFILE_STABILITY_MISSING_LOG"
  cat "$PROFILE_STABILITY_MISSING_SUMMARY"
  exit 1
fi
...
echo "[single-machine-prod-readiness] profile compare multi-vm stability check summary (valid)"
cat >"$MULTI_VM_STABILITY_CHECK_VALID_SOURCE_JSON" <<'EOF_MULTI_VM_STABILITY_CHECK_VALID'
{
  "version": 1,
  "schema": {
...
EOF_MULTI_VM_STABILITY_CHECK_VALID
FAKE_MANUAL_REPORT_MODE=pending_multi \
FAKE_MANUAL_REPORT_MULTI_VM_STABILITY_CHECK_SUMMARY_JSON="$MULTI_VM_STABILITY_CHECK_VALID_SOURCE_JSON" \
SINGLE_MACHINE_CI_LOCAL_SCRIPT="$FAKE_CI" \
SINGLE_MACHINE_BETA_PREFLIGHT_SCRIPT="$FAKE_BETA" \
SINGLE_MACHINE_DEEP_TEST_SUITE_SCRIPT="$FAKE_DEEP_OK" \
SINGLE_MACHINE_RUNTIME_FIX_RECORD_SCRIPT="$FAKE_RUNTIME_FIX_RECORD" \
...
./scripts/single_machine_prod_readiness.sh \
  ...
  --run-pre-real-host-readiness 0 \
  --run-real-wg-privileged-matrix 0 \
  --profile-compare-campaign-signoff-reports-dir "$TMP_DIR/multi_vm_stability_check_valid_reports" \
  --summary-json "$MULTI_VM_STABILITY_CHECK_VALID_SUMMARY" \
  --print-summary-json 0 >"$MULTI_VM_STABILITY_CHECK_VALID_LOG"
if ! jq -e --arg multi_vm_json "$MULTI_VM_STABILITY_CHECK_VALID_SOURCE_JSON" '
  .summary.profile_default_gate.multi_vm_stability_check_summary_json == $multi_vm_json
  ...
  and .summary.profile_default_gate.multi_vm_stability_check_rc == 0
  and .summary.profile_default_gate.multi_vm_stability_check_modal_recommended_profile == "balanced"
  and .summary.profile_default_gate.multi_vm_stability_check_modal_support_rate_pct == 66.67
' "$MULTI_VM_STABILITY_CHECK_VALID_SUMMARY" >/dev/null; then
  echo "valid multi-vm stability check summary did not propagate as expected"
  cat "$MULTI_VM_STABILITY_CHECK_VALID_LOG"
  cat "$MULTI_VM_STABILITY_CHECK_VALID_SUMMARY"
  exit 1
fi
if ! rg -q '\[single-machine-prod-readiness\] profile_default_gate_multi_vm_stability_check_available=true' "$MULTI_VM_STABILITY_CHECK_VALID_LOG"; then
  echo "valid multi-vm stability check path missing availability=true log line"
  cat "$MULTI_VM_STABILITY_CHECK_VALID_LOG"
  exit 1
fi
if ! rg -q '\[single-machine-prod-readiness\] profile_default_gate_multi_vm_stability_check_status=ok decision=GO go=1 no_go=0 rc=0 modal_profile=balanced modal_support_rate_pct=66.67' "$MULTI_VM_STABILITY_CHECK_VALID_LOG"; then
  echo "valid multi-vm stability check path missing status log line"
  cat "$MULTI_VM_STABILITY_CHECK_VALID_LOG"
  exit 1
fi

echo "[single-machine-prod-readiness] profile compare multi-vm stability check summary (invalid shape)"
cat >"$MULTI_VM_STABILITY_CHECK_INVALID_SOURCE_JSON" <<'EOF_MULTI_VM_STABILITY_CHECK_INVALID'
{
  "version": 1,
  "schema": {
    "id": "wrong_schema"
...
EOF_MULTI_VM_STABILITY_CHECK_INVALID
FAKE_MANUAL_REPORT_MODE=pending_multi \
FAKE_MANUAL_REPORT_MULTI_VM_STABILITY_CHECK_SUMMARY_JSON="$MULTI_VM_STABILITY_CHECK_INVALID_SOURCE_JSON" \
SINGLE_MACHINE_CI_LOCAL_SCRIPT="$FAKE_CI" \
SINGLE_MACHINE_BETA_PREFLIGHT_SCRIPT="$FAKE_BETA" \
SINGLE_MACHINE_DEEP_TEST_SUITE_SCRIPT="$FAKE_DEEP_OK" \
SINGLE_MACHINE_RUNTIME_FIX_RECORD_SCRIPT="$FAKE_RUNTIME_FIX_RECORD" \
...
./scripts/single_machine_prod_readiness.sh \
  ...
  --run-pre-real-host-readiness 0 \
  --run-real-wg-privileged-matrix 0 \
  --profile-compare-campaign-signoff-reports-dir "$TMP_DIR/multi_vm_stability_check_invalid_reports" \
  --summary-json "$MULTI_VM_STABILITY_CHECK_INVALID_SUMMARY" \
  --print-summary-json 0 >"$MULTI_VM_STABILITY_CHECK_INVALID_LOG"
if ! jq -e --arg multi_vm_json "$MULTI_VM_STABILITY_CHECK_INVALID_SOURCE_JSON" '
  .summary.profile_default_gate.multi_vm_stability_check_summary_json == $multi_vm_json
  and .summary.profile_default_gate.multi_vm_stability_check_summary_available == false
  and .summary.profile_default_gate.multi_vm_stability_check_decision == null
  and .summary.profile_default_gate.multi_vm_stability_check_status == null
  and .summary.profile_default_gate.multi_vm_stability_check_go == null
  and .summary.profile_default_gate.multi_vm_stability_check_no_go == null
  and .summary.profile_default_gate.multi_vm_stability_check_rc == null
  and .summary.profile_default_gate.multi_vm_stability_check_modal_recommended_profile == null
  and .summary.profile_default_gate.multi_vm_stability_check_modal_support_rate_pct == null
' "$MULTI_VM_STABILITY_CHECK_INVALID_SUMMARY" >/dev/null; then
  echo "invalid multi-vm stability check summary should fail closed"
  cat "$MULTI_VM_STABILITY_CHECK_INVALID_LOG"
  cat "$MULTI_VM_STABILITY_CHECK_INVALID_SUMMARY"
  exit 1
fi
if ! rg -q '\[single-machine-prod-readiness\] profile_default_gate_multi_vm_stability_check_available=false' "$MULTI_VM_STABILITY_CHECK_INVALID_LOG"; then
  echo "invalid multi-vm stability check path missing availability=false log line"
  cat "$MULTI_VM_STABILITY_CHECK_INVALID_LOG"
  exit 1
fi

echo "[single-machine-prod-readiness] profile compare multi-vm stability check summary (missing default path)"
rm -rf "$MULTI_VM_STABILITY_CHECK_MISSING_REPORTS_DIR"
FAKE_MANUAL_REPORT_MODE=pending_multi \
SINGLE_MACHINE_CI_LOCAL_SCRIPT="$FAKE_CI" \
SINGLE_MACHINE_BETA_PREFLIGHT_SCRIPT="$FAKE_BETA" \
SINGLE_MACHINE_DEEP_TEST_SUITE_SCRIPT="$FAKE_DEEP_OK" \
...
./scripts/single_machine_prod_readiness.sh \
  ...
  --run-pre-real-host-readiness 0 \
  --run-real-wg-privileged-matrix 0 \
  --profile-compare-campaign-signoff-reports-dir "$MULTI_VM_STABILITY_CHECK_MISSING_REPORTS_DIR" \
  --summary-json "$MULTI_VM_STABILITY_CHECK_MISSING_SUMMARY" \
  --print-summary-json 0 >"$MULTI_VM_STABILITY_CHECK_MISSING_LOG"
if ! jq -e --arg multi_vm_json "$MULTI_VM_STABILITY_CHECK_MISSING_DEFAULT_JSON" '
  .summary.profile_default_gate.multi_vm_stability_check_summary_json == $multi_vm_json
  and .summary.profile_default_gate.multi_vm_stability_check_summary_available == false
  and .summary.profile_default_gate.multi_vm_stability_check_decision == null
  and .summary.profile_default_gate.multi_vm_stability_check_status == null
  ...
  and .summary.profile_default_gate.multi_vm_stability_check_modal_recommended_profile == null
  and .summary.profile_default_gate.multi_vm_stability_check_modal_support_rate_pct == null
' "$MULTI_VM_STABILITY_CHECK_MISSING_SUMMARY" >/dev/null; then
  echo "missing default multi-vm stability check summary should leave additive fields null-safe"
  cat "$MULTI_VM_STABILITY_CHECK_MISSING_LOG"
  cat "$MULTI_VM_STABILITY_CHECK_MISSING_SUMMARY"
  exit 1
fi
if ! rg -q '\[single-machine-prod-readiness\] profile_default_gate_multi_vm_stability_check_available=false' "$MULTI_VM_STABILITY_CHECK_MISSING_LOG"; then
  echo "missing multi-vm stability check path missing availability=false log line"
  cat "$MULTI_VM_STABILITY_CHECK_MISSING_LOG"
  exit 1
fi

echo "[single-machine-prod-readiness] profile default gate stability cycle summary (valid)"
cat >"$PROFILE_STABILITY_CYCLE_VALID_SOURCE_JSON" <<'EOF_STABILITY_CYCLE_VALID'
{
  "version": 1,
  "schema": {
...
EOF_STABILITY_CYCLE_VALID
FAKE_MANUAL_REPORT_MODE=pending_multi \
FAKE_MANUAL_REPORT_STABILITY_CYCLE_SUMMARY_JSON="$PROFILE_STABILITY_CYCLE_VALID_SOURCE_JSON" \
SINGLE_MACHINE_CI_LOCAL_SCRIPT="$FAKE_CI" \
SINGLE_MACHINE_BETA_PREFLIGHT_SCRIPT="$FAKE_BETA" \
SINGLE_MACHINE_DEEP_TEST_SUITE_SCRIPT="$FAKE_DEEP_OK" \
SINGLE_MACHINE_RUNTIME_FIX_RECORD_SCRIPT="$FAKE_RUNTIME_FIX_RECORD" \
...
./scripts/single_machine_prod_readiness.sh \
  ...
  --run-pre-real-host-readiness 0 \
  --run-real-wg-privileged-matrix 0 \
  --profile-compare-campaign-signoff-reports-dir "$TMP_DIR/profile_default_gate_stability_cycle_valid_reports" \
  --summary-json "$PROFILE_STABILITY_CYCLE_VALID_SUMMARY" \
  --print-summary-json 0 >"$PROFILE_STABILITY_CYCLE_VALID_LOG"
if ! jq -e --arg cycle_json "$PROFILE_STABILITY_CYCLE_VALID_SOURCE_JSON" '
  .summary.profile_default_gate.cycle_summary_json == $cycle_json
...
' "$PROFILE_STABILITY_CYCLE_VALID_SUMMARY" >/dev/null; then
  echo "valid profile default gate stability cycle summary did not propagate as expected"
  cat "$PROFILE_STABILITY_CYCLE_VALID_LOG"
  cat "$PROFILE_STABILITY_CYCLE_VALID_SUMMARY"
  exit 1
fi
if ! rg -q '\[single-machine-prod-readiness\] profile_default_gate_stability_cycle_available=true' "$PROFILE_STABILITY_CYCLE_VALID_LOG"; then
  echo "valid stability cycle path missing availability=true log line"
  cat "$PROFILE_STABILITY_CYCLE_VALID_LOG"
  exit 1
fi
if ! rg -q '\[single-machine-prod-readiness\] profile_default_gate_stability_cycle_status=pass decision=GO rc=0 failure_stage=unset failure_reason=unset' "$PROFILE_STABILITY_CYCLE_VALID_LOG"; then
  echo "valid stability cycle path missing status log line"
  cat "$PROFILE_STABILITY_CYCLE_VALID_LOG"
  exit 1
fi

echo "[single-machine-prod-readiness] profile default gate stability cycle summary (invalid shape)"
cat >"$PROFILE_STABILITY_CYCLE_INVALID_SOURCE_JSON" <<'EOF_STABILITY_CYCLE_INVALID'
{
  "version": 1,
  "schema": {
    "id": "wrong_schema"
...
EOF_STABILITY_CYCLE_INVALID
FAKE_MANUAL_REPORT_MODE=pending_multi \
FAKE_MANUAL_REPORT_STABILITY_CYCLE_SUMMARY_JSON="$PROFILE_STABILITY_CYCLE_INVALID_SOURCE_JSON" \
SINGLE_MACHINE_CI_LOCAL_SCRIPT="$FAKE_CI" \
SINGLE_MACHINE_BETA_PREFLIGHT_SCRIPT="$FAKE_BETA" \
SINGLE_MACHINE_DEEP_TEST_SUITE_SCRIPT="$FAKE_DEEP_OK" \
SINGLE_MACHINE_RUNTIME_FIX_RECORD_SCRIPT="$FAKE_RUNTIME_FIX_RECORD" \
...
./scripts/single_machine_prod_readiness.sh \
  ...
  --run-pre-real-host-readiness 0 \
  --run-real-wg-privileged-matrix 0 \
  --profile-compare-campaign-signoff-reports-dir "$TMP_DIR/profile_default_gate_stability_cycle_invalid_reports" \
  --summary-json "$PROFILE_STABILITY_CYCLE_INVALID_SUMMARY" \
  --print-summary-json 0 >"$PROFILE_STABILITY_CYCLE_INVALID_LOG"
if ! jq -e --arg cycle_json "$PROFILE_STABILITY_CYCLE_INVALID_SOURCE_JSON" '
  .summary.profile_default_gate.cycle_summary_json == $cycle_json
  ...
  and .summary.profile_default_gate.cycle_failure_stage == null
  and .summary.profile_default_gate.cycle_failure_reason == null
' "$PROFILE_STABILITY_CYCLE_INVALID_SUMMARY" >/dev/null; then
  echo "invalid profile default gate stability cycle summary should fail closed"
  cat "$PROFILE_STABILITY_CYCLE_INVALID_LOG"
  cat "$PROFILE_STABILITY_CYCLE_INVALID_SUMMARY"
  exit 1
fi
if ! rg -q '\[single-machine-prod-readiness\] profile_default_gate_stability_cycle_available=false' "$PROFILE_STABILITY_CYCLE_INVALID_LOG"; then
  echo "invalid stability cycle path missing availability=false log line"
  cat "$PROFILE_STABILITY_CYCLE_INVALID_LOG"
  exit 1
fi

echo "[single-machine-prod-readiness] profile default gate stability cycle summary (missing default path)"
rm -rf "$PROFILE_STABILITY_CYCLE_MISSING_REPORTS_DIR"
FAKE_MANUAL_REPORT_MODE=pending_multi \
SINGLE_MACHINE_CI_LOCAL_SCRIPT="$FAKE_CI" \
SINGLE_MACHINE_BETA_PREFLIGHT_SCRIPT="$FAKE_BETA" \
SINGLE_MACHINE_DEEP_TEST_SUITE_SCRIPT="$FAKE_DEEP_OK" \
...
./scripts/single_machine_prod_readiness.sh \
  ...
  --run-pre-real-host-readiness 0 \
  --run-real-wg-privileged-matrix 0 \
  --profile-compare-campaign-signoff-reports-dir "$PROFILE_STABILITY_CYCLE_MISSING_REPORTS_DIR" \
  --summary-json "$PROFILE_STABILITY_CYCLE_MISSING_SUMMARY" \
  --print-summary-json 0 >"$PROFILE_STABILITY_CYCLE_MISSING_LOG"
if ! jq -e --arg cycle_json "$PROFILE_STABILITY_CYCLE_MISSING_DEFAULT_JSON" '
  .summary.profile_default_gate.cycle_summary_json == $cycle_json
  and .summary.profile_default_gate.cycle_summary_available == false
  and .summary.profile_default_gate.cycle_decision == null
  and .summary.profile_default_gate.cycle_status == null
  ...
  and .summary.profile_default_gate.cycle_failure_stage == null
  and .summary.profile_default_gate.cycle_failure_reason == null
' "$PROFILE_STABILITY_CYCLE_MISSING_SUMMARY" >/dev/null; then
  echo "missing default stability cycle summary should leave additive fields null-safe"
  cat "$PROFILE_STABILITY_CYCLE_MISSING_LOG"
  cat "$PROFILE_STABILITY_CYCLE_MISSING_SUMMARY"
  exit 1
fi
if ! rg -q '\[single-machine-prod-readiness\] profile_default_gate_stability_cycle_available=false' "$PROFILE_STABILITY_CYCLE_MISSING_LOG"; then
  echo "missing stability cycle path missing availability=false log line"
  cat "$PROFILE_STABILITY_CYCLE_MISSING_LOG"
  exit 1
fi
...
echo "[single-machine-prod-readiness] auto refresh invalid generated_at_utc does not fallback to mtime"
rm -rf "$TMP_DIR/auto_refresh_invalid_generated_at_reports"
mkdir -p "$TMP_DIR/auto_refresh_invalid_generated_at_reports"
cat >"$TMP_DIR/auto_refresh_invalid_generated_at_reports/profile_compare_campaign_signoff_summary.json" <<'EOF_INVALID_GENERATED_AT_SIGNOFF'
{
  "version": 1,
...
EOF_INVALID_GENERATED_AT_SIGNOFF
# Keep filesystem mtime fresh to prove invalid embedded timestamps fail closed.
touch "$TMP_DIR/auto_refresh_invalid_generated_at_reports/profile_compare_campaign_signoff_summary.json"
: >"$PROFILE_SIGNOFF_ARGS_LOG"
FAKE_MANUAL_REPORT_MODE=pending_multi \
SINGLE_MACHINE_CI_LOCAL_SCRIPT="$FAKE_CI" \
SINGLE_MACHINE_BETA_PREFLIGHT_SCRIPT="$FAKE_BETA" \
...
./scripts/single_machine_prod_readiness.sh \
  ...
  --profile-compare-campaign-signoff-refresh-campaign 0 \
  --profile-compare-campaign-signoff-summary-max-age-sec 3600 \
  --profile-compare-campaign-signoff-reports-dir "$TMP_DIR/auto_refresh_invalid_generated_at_reports" \
  --profile-compare-campaign-signoff-summary-json "$TMP_DIR/auto_refresh_invalid_generated_at_reports/profile_compare_campaign_signoff_summary.json" \
  --run-pre-real-host-readiness 0 \
  --run-real-wg-privileged-matrix 0 \
  --summary-json "$AUTO_REFRESH_INVALID_GENERATED_AT_SUMMARY" \
  --print-summary-json 0 >"$AUTO_REFRESH_INVALID_GENERATED_AT_LOG"
...
if ! jq -e '
  ...
  and .inputs.profile_compare_campaign_signoff_existing_summary.decision == "GO"
  and .inputs.profile_compare_campaign_signoff_existing_summary.refresh_campaign == true
  and .inputs.profile_compare_campaign_signoff_existing_summary.generated_at_utc == "not-a-timestamp"
  and .inputs.profile_compare_campaign_signoff_existing_summary.freshness_source == null
  and .inputs.profile_compare_campaign_signoff_existing_summary.freshness_available == false
  and .inputs.profile_compare_campaign_signoff_existing_summary.fresh == null
  and .inputs.profile_compare_campaign_signoff_existing_summary.requires_refresh == true
  and ((.inputs.profile_compare_campaign_signoff_existing_summary.refresh_reason // "") | startswith("invalid generated_at_utc in signoff summary"))
  and (.steps[] | select(.step_id == "profile_compare_campaign_signoff") | .status == "pass")
' "$AUTO_REFRESH_INVALID_GENERATED_AT_SUMMARY" >/dev/null; then
  echo "invalid generated_at_utc freshness path should fail closed despite fresh mtime"
  cat "$AUTO_REFRESH_INVALID_GENERATED_AT_LOG"
  cat "$AUTO_REFRESH_INVALID_GENERATED_AT_SUMMARY"
  exit 1
fi
if ! rg -q -- '--refresh-campaign 1' "$PROFILE_SIGNOFF_ARGS_LOG"; then
  echo "invalid generated_at_utc path did not force --refresh-campaign 1"
  cat "$PROFILE_SIGNOFF_ARGS_LOG"
  exit 1
fi

echo "[single-machine-prod-readiness] auto refresh future generated_at_utc fails closed"
rm -rf "$TMP_DIR/auto_refresh_future_generated_at_reports"
mkdir -p "$TMP_DIR/auto_refresh_future_generated_at_reports"
cat >"$TMP_DIR/auto_refresh_future_generated_at_reports/profile_compare_campaign_signoff_summary.json" <<'EOF_FUTURE_GENERATED_AT_SIGNOFF'
{
  "version": 1,
...
EOF_FUTURE_GENERATED_AT_SIGNOFF
: >"$PROFILE_SIGNOFF_ARGS_LOG"
FAKE_MANUAL_REPORT_MODE=pending_multi \
SINGLE_MACHINE_CI_LOCAL_SCRIPT="$FAKE_CI" \
SINGLE_MACHINE_BETA_PREFLIGHT_SCRIPT="$FAKE_BETA" \
...
./scripts/single_machine_prod_readiness.sh \
  ...
  --profile-compare-campaign-signoff-refresh-campaign 0 \
  --profile-compare-campaign-signoff-summary-max-age-sec 3600 \
  --profile-compare-campaign-signoff-reports-dir "$TMP_DIR/auto_refresh_future_generated_at_reports" \
  --profile-compare-campaign-signoff-summary-json "$TMP_DIR/auto_refresh_future_generated_at_reports/profile_compare_campaign_signoff_summary.json" \
  --run-pre-real-host-readiness 0 \
  --run-real-wg-privileged-matrix 0 \
  --summary-json "$AUTO_REFRESH_FUTURE_GENERATED_AT_SUMMARY" \
  --print-summary-json 0 >"$AUTO_REFRESH_FUTURE_GENERATED_AT_LOG"
...
if ! jq -e '
  ...
  and .inputs.profile_compare_campaign_signoff_existing_summary.decision == "GO"
  and .inputs.profile_compare_campaign_signoff_existing_summary.refresh_campaign == true
  and .inputs.profile_compare_campaign_signoff_existing_summary.generated_at_utc == "2099-01-01T00:00:00Z"
  and .inputs.profile_compare_campaign_signoff_existing_summary.freshness_source == null
  and .inputs.profile_compare_campaign_signoff_existing_summary.freshness_available == false
  and .inputs.profile_compare_campaign_signoff_existing_summary.fresh == null
  and .inputs.profile_compare_campaign_signoff_existing_summary.requires_refresh == true
  and ((.inputs.profile_compare_campaign_signoff_existing_summary.refresh_reason // "") | startswith("future generated_at_utc in signoff summary"))
  and (.steps[] | select(.step_id == "profile_compare_campaign_signoff") | .status == "pass")
' "$AUTO_REFRESH_FUTURE_GENERATED_AT_SUMMARY" >/dev/null; then
  echo "future generated_at_utc freshness path should fail closed"
  cat "$AUTO_REFRESH_FUTURE_GENERATED_AT_LOG"
  cat "$AUTO_REFRESH_FUTURE_GENERATED_AT_SUMMARY"
  exit 1
fi
if ! rg -q -- '--refresh-campaign 1' "$PROFILE_SIGNOFF_ARGS_LOG"; then
  echo "future generated_at_utc path did not force --refresh-campaign 1"
  cat "$PROFILE_SIGNOFF_ARGS_LOG"
  exit 1
fi
