...
echo "[profile-default-gate-stability-check] decision=$decision status=$status rc=$rc modal_profile=${observed_modal_recommended_profile:-unset} modal_support_rate_pct=${observed_modal_support_rate_pct}"
if ((${#errors[@]} > 0)); then
