...
TMP_DIR="$(mktemp -d)"
LOG_FILE="$TMP_DIR/session_handoff.log"
...
ENTRY_PORT=19183
EXIT_PORT=19184
ENTRY_DATA_PORT=20180
EXIT_DATA_PORT=20181
EXIT_WG_PORT=20182
...
for _ in $(seq 1 120); do
  if rg -q "client session handoff (complete|cleanup .* )old_session=.* new_session=" "$LOG_FILE"; then
    handoff_ok=1
    break
  fi
  sleep 0.2
...
handoff_line=$(rg -N "client session handoff (complete|cleanup .* )old_session=.* new_session=" "$LOG_FILE" | tail -n1)
