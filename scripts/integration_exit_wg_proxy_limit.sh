...
chmod 600 "$KEY_EXIT" "$KEY_C1" "$KEY_C2"
...
DIRECTORY_PUBLIC_URL=http://127.0.0.1:8311 \
...
ISSUER_URLS=http://127.0.0.1:8312 \
CORE_ISSUER_URL=http://127.0.0.1:8312 \
...
wait_client_cfg() {
  ...
  start="$(date +%s)"
  while true; do
    if rg -q "client wireguard runtime ready:|client wg-kernel proxy listening:" "$log"; then
      return 0
    fi
    if (( $(date +%s) - start >= timeout_sec )); then
      return 1
