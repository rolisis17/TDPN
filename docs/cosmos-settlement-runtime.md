...
- `blockchain_gate_bundle` runs a Cosmos-only guardrail pre-stage before activation-metrics + mainnet-activation-gate + bootstrap-governance-graduation-gate stages; default `enforce` mode forwards real-evidence and fail-close settings for launch approval, while `--report-only` preserves fail-soft diagnostic summary generation.
