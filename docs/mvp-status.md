...
- Easy-mode wrappers for host WireGuard validation (`easy_node.sh wg-only-check`, `easy_node.sh wg-only-local-test`) plus reusable host stack lifecycle (`easy_node.sh wg-only-stack-up|status|down`) and launcher menu wiring.
