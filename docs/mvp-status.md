...
- Easy-mode wrappers for host WireGuard validation (`easy_node.sh wg-only-check`, `easy_node.sh wg-only-local-test`) plus reusable host stack lifecycle (`easy_node.sh wg-only-stack-up|status|down`), one-command lifecycle validation (`easy_node.sh wg-only-stack-selftest`, `integration_wg_only_stack_selftest.sh`), and launcher menu wiring.
