...
`text-export` and `text-import` validate that the payload matches its envelope kind, so a raw helper registry cannot be mislabeled as a signed helper registry handoff.
