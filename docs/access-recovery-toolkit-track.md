...
- `access_bridge_service_smoke.sh` records deployed bridge health, observed transport/TLS facts, optional required-mTLS proof, access-code-gated path availability, helper/org/registry identity, security headers, and abuse endpoint acceptance into a JSON summary
...
- if the helper proxy is enforcing client certificates, run the evidence command with `--require-mtls 1 --cacert CA.crt --client-cert CLIENT.crt --client-key CLIENT.key`; the summary must show the client-cert probe succeeded and the no-client-cert probe was rejected on the same endpoint
