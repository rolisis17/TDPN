...
`access-recovery-real-helper-evidence-run --roadmap-refresh 0` is diagnostics/verifier-only. It can inspect the verifier path, but it skips roadmap/status refresh and is not full pilot handoff completion.
