...
- `scripts/roadmap_progress_report.sh` now consumes optional phase6 summary artifacts and surfaces `phase6_cosmos_l1_handoff` signals (`run_pipeline_ok`, `module_tx_surface_ok`, `tdpnd_grpc_runtime_smoke_ok`, `tdpnd_grpc_live_smoke_ok`, `tdpnd_grpc_auth_live_smoke_ok`) under `blockchain_track`, with integration coverage in `scripts/integration_roadmap_progress_report.sh`.
