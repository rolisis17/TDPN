...
- GPM Admin Console settlement evidence now has a one-command capture path: `./scripts/easy_node.sh gpm-admin-settlement-live-evidence` runs the settlement/slashing proof contract against a configured tdpnd bridge, or a temporary local tdpnd bridge for repeatable validation, and emits `gpm_admin_settlement_live_evidence_summary`. It verifies objective reward-proof registration/query, reservation/settlement finality, weekly payout finality, slash-evidence hold fail-closed behavior, slash-evidence material-mismatch finality rejection, and penalty acceptance only after slash-evidence finality; bridge finality also rejects slash-evidence confirmation when optional submitted material mismatches stored evidence.
...
- `ci_phase0.sh` is now the fast Phase-0 product-surface gate runner for launcher wiring/runtime, simple prompt-budget contract (`<=6` prompts), config-v1, local control API, public/admin split, desktop/Admin Console release guardrails, GPM admin settlement docs, and lightweight Access Recovery checked examples; use it for quick contract checks before full `ci_local`/preflight runs.
...
- enforce the handoff-authority verifier contract: signed trusted provenance, real-helper HTTPS, installed-host evidence, schema minor `>=5`, `handoff_authority=true`, `authority_level=pilot_handoff`, `integrity_only=false`, and a trust-store SHA; dev/public-key-only trust and local rehearsal are non-authoritative
...
- a trusted verifier receipt can claim handoff authority only with schema minor `>=5`, signed trusted provenance, real-helper HTTPS, installed-host evidence, `handoff_authority=true`, `authority_level=pilot_handoff`, `integrity_only=false`, and a trust-store SHA
