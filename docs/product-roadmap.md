...
- GPM wallet-auth local evidence is now explicit about scope: `./scripts/easy_node.sh gpm-wallet-auth-evidence` proves deterministic local secp256k1 binding, Keplr/Leap-shaped alias handling, mismatch/admin-elevation rejection, and command-backed admin separation, while guarding against vacuous `go test -run` passes and keeping installed Keplr/Leap browser-extension beta evidence as a pending release artifact.
