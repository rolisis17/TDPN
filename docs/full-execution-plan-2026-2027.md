...
- Operator guidance for the activation metrics evidence cycle: generate a canonical metrics template first, fill measured values/evidence links, then run the activation gate cycle helper to execute normalization + metrics evaluation + gate decision as one repeatable operator pass; report-only diagnostics keep explicit `NO-GO` reasons, while launch approval must use enforce mode with real evidence. VPN dataplane forwarding remains independent from chain/gate liveness.
- Blockchain gate bundle helper is `scripts/blockchain_gate_bundle.sh`; easy-node entrypoint is `./scripts/easy_node.sh blockchain-gate-bundle`; integration contract is `scripts/integration_blockchain_gate_bundle.sh`; it composes activation-metrics + mainnet-activation-gate + bootstrap-governance-graduation-gate into one contract run, emits canonical bundle summary output, defaults to enforce mode for launch approval (real evidence plus fail-closed activation/bootstrap gates), and preserves fail-soft summary generation only through explicit `--report-only`; VPN dataplane independence remains unchanged.
...
- Phase 7 cutover/handoff signal snapshots include `mainnet_activation_gate_go` and `bootstrap_governance_graduation_gate_go`; these requirements are required by default for handoff readiness and can only be relaxed explicitly with the `--require-* 0` gate flags in non-production dry-run/support scenarios.
