...
- `scripts/blockchain_mainnet_activation_gate.sh` evaluates the metrics gate from a bootstrap metrics JSON file and emits one summary JSON with the final GO/NO-GO decision; its default `enforce` mode requires source-json provenance and fails closed on `NO-GO`/missing/invalid input. Use `--report-only` only for diagnostics that must keep emitting summaries without shell-exit approval semantics.
- `scripts/integration_blockchain_mainnet_activation_gate.sh` covers the enforce path, report-only GO/NO-GO paths, missing/invalid input handling, and fail-close behavior.
...
- Operator workflow should use the activation metrics template helper and activation gate cycle helper together so template generation, normalization, metrics evaluation, and final gate decision run as one repeatable loop; report-only diagnostics can stay fail-soft (`NO-GO` with explicit reasons), while launch approval must use enforce mode with real evidence. VPN dataplane behavior remains independent from chain/gate liveness.
