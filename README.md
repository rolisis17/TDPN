...
- `node scripts/integration_recovery_browser_smoke.js` (browser-local recovery page smoke using a generated signed bridge invite and signed helper registry)
...
- `go run ./cmd/gpmrecover trust-export-key --trust-store .easy-node-logs/access-recovery-demo/recovery-trust.json --org-id freenews-demo --key-id KEY_ID --text-out .easy-node-logs/access-recovery-demo/recovery-trusted-key.txt` (export one trusted organization key as a first-time trust handoff)
