...
- `go run ./cmd/gpmrecover bridge-service-serve --config .easy-node-logs/access-recovery-demo/bridge-service-config.json --addr 127.0.0.1:18980 --rps 2 --abuse-log .easy-node-logs/access-recovery-demo/bridge-abuse.jsonl --access-code-sha256 HASH` (minimal helper HTTP wrapper with `/health`, `/bridge/{path_id}`, optional ticket/code gate, per-source limits, and `/abuse` logging)
- `go run ./cmd/gpmrecover bridge-service-deploy-pack --out-dir .easy-node-logs/access-recovery-demo/bridge-deploy --public-host bridge.example --access-code-sha256 HASH` (generate env, run-wrapper, hardened systemd unit, and Caddy/nginx HTTPS examples for a helper bridge)
