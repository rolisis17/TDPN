...
- `./scripts/easy_node.sh access-recovery-beta-local-gate --reports-dir .easy-node-logs/access-recovery-beta-local-gate --print-summary-json 1` (focused local Access Recovery beta gate: demo/examples, browser verifier, bridge service, deployment evidence, host install check, pilot bundle generation, and bundle verification without the legacy VPN matrix)
- `./scripts/easy_node.sh access-recovery-local-evidence-refresh --write-canonical 1 --refresh-roadmap 1 --print-summary-json 1` (local loopback Access Recovery rehearsal that writes canonical roadmap evidence; real pilot handoff still requires helper HTTPS evidence)
