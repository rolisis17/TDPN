...
- `./scripts/easy_node.sh access-recovery-local-evidence-refresh --write-canonical 1 --refresh-roadmap 1 --print-summary-json 1` refreshes local loopback rehearsal evidence; `roadmap-next-actions-run --include real_helper_https_evidence` is the operator-input materialization path for real-helper evidence; real pilot handoff still requires real helper HTTPS evidence, signed provenance, trusted verification, and a verifier receipt.
