...
- `./scripts/easy_node.sh access-recovery-local-evidence-refresh --write-canonical 1 --refresh-roadmap 1 --print-summary-json 1` refreshes local loopback rehearsal evidence; `roadmap-next-actions-run --include-id real_helper_https_evidence` is the operator-input materialization path for real-helper evidence; real pilot handoff still requires real helper HTTPS evidence, signed provenance, trusted verification, and a verifier receipt.
...
# optional: persist machine-readable WG validate summary JSON
...
- `prod-pilot-cohort-check` validates cohort summary policy (status/round failures/trend go-rate/alert severity/bundle requirements) for fail-close GO/NO-GO decisions, enforces fresh evidence by default (`--max-evidence-age-sec`, default `600`), and fails closed if summary trend/alert metrics disagree with the linked trend/alert artifacts.
...
- `prod-pilot-cohort-signoff` runs `prod-pilot-cohort-bundle-verify` + `prod-pilot-cohort-check` in one fail-closed command and rejects bundle tar/checksum/manifest overrides that do not match the summary artifact paths.
...
- `prod-pilot-cohort-quick-trend` aggregates GO/NO-GO trend across quick run-report artifacts with fail-close thresholds and JSON output, treats malformed/missing quick-check decisions as `NO-GO` evaluation errors, and now carries latest failed incident handoff paths, attachment manifest pointers, and the upstream `pre_real_host_readiness_summary_json` pointer when available.
- `prod-pilot-cohort-quick-alert` maps quick trend metrics to operator severity (`OK`/`WARN`/`CRITICAL`) with optional fail-close exits, validates supplied trend summaries against their `.runs[]` evidence before trusting severity, and now preserves the same incident handoff attachment pointers plus the upstream `pre_real_host_readiness_summary_json` pointer in alert JSON/output.
...
- `prod-pilot-cohort-quick-signoff` runs quick-check + quick-trend + quick-alert in one fail-closed command, requires trend when alerting is enabled, enforces `--max-alert-severity` policy, and carries both incident handoff artifact paths and the upstream `pre_real_host_readiness_summary_json` path into its signoff JSON/output when present.
...
- `prod-mtls-bundle-verify --bundle-dir <HOST_BUNDLE_DIR> --host <PUBLIC_IP_OR_DNS>` is the pre-cutover leaf-bundle check for each server. It verifies `ca.crt`, `node.crt`, `node.key`, certificate chain, serverAuth, key pairing, expiry, and host SAN coverage, and fails by default if `ca.key` was accidentally staged with the server bundle.
- `prod-mtls-bundle-stage --bundle-dir <HOST_BUNDLE_DIR> --host <PUBLIC_IP_OR_DNS>` stages a verified per-host bundle into `deploy/tls` without restarting services and removes `ca.key` from the server target. Use `server-up --prod-profile 1 --prod-mtls-mode staged` only after staging; default prod server-up still uses bootstrap mode for compatibility.
...
- `docs/access-recovery-operator-runbook.md` (first beta operator/helper flow for key handoff, bridge-service install checklist, helper onboarding, quarantine, expiry, and incident response)
- `go run ./cmd/gpmrecover demo-bundle --out-dir .easy-node-logs/access-recovery-demo --org-id freenews-demo --org-name "FreeNews Demo" --helper-id helper-demo --helper-name "Demo bridge helper"` (one-command local Access Recovery demo/rehearsal artifacts)
- `bash ./scripts/integration_recovery_browser_smoke.sh` (browser-local recovery page smoke using a generated signed bridge invite and signed helper registry)
- `bash ./scripts/integration_access_recovery_demo_contract.sh` (operator/CLI contract for generated demo bundle, trusted-key handoffs, signed registry policy, and text import paths)
- `bash ./scripts/integration_access_bridge_service_serve.sh` (runs the minimal bridge HTTP service and checks health, ticket-code gating, security headers, and abuse logging)
- `./scripts/easy_node.sh access-recovery-beta-local-gate --reports-dir .easy-node-logs/access-recovery-beta-local-gate --print-summary-json 1` (focused local Access Recovery beta gate for contract/local coverage only: demo/examples, browser verifier, bridge service, local/dev deployment evidence contracts, host install check, pilot bundle generation, and trusted verifier receipt contract without the legacy VPN matrix; real pilot handoff still requires real helper HTTPS evidence, signed provenance, and trusted verification)
- `./scripts/easy_node.sh access-recovery-local-evidence-refresh --write-canonical 1 --refresh-roadmap 1 --print-summary-json 1` (local loopback Access Recovery rehearsal that writes canonical roadmap evidence; real pilot handoff still requires real helper HTTPS evidence, signed provenance, trusted verification, and a verifier receipt bound to the verified bundle evidence hashes and child-summary semantics)
- `./scripts/easy_node.sh access-recovery-real-helper-evidence-run --base-url https://HELPER_PUBLIC_DNS --path-id helper-web --code-file PRIVATE_CODE_FILE --config-json BRIDGE_SERVICE_CONFIG --deploy-pack-dir BRIDGE_DEPLOY_PACK --host-install-evidence-mode installed-host --install-dir /etc/gpm/access-bridge --systemd-unit-file /etc/systemd/system/gpm-access-bridge.service --proxy-kind caddy --proxy-config-file /etc/caddy/Caddyfile.d/gpm-access-bridge.caddy --expect-helper-id HELPER_ID --expect-org-id ORG_ID --provenance-private-key-file PROVENANCE_PRIVATE_KEY_FILE --provenance-org-id ORG_ID --provenance-org-name ORG_NAME --trust-store TRUST_STORE --reports-dir .easy-node-logs/access-recovery-pilot` (preferred real-helper handoff command: refuses placeholders/private helper URLs, captures signed public HTTPS evidence, records installed-host service/proxy evidence, writes trusted verifier receipt, and refreshes roadmap readiness)
- `bash ./scripts/access_bridge_service_smoke.sh --base-url https://HELPER_PUBLIC_DNS --path-id helper-web --code-file PRIVATE_CODE_FILE --expect-helper-id HELPER_ID --expect-org-id ORG_ID --summary-json .easy-node-logs/bridge-service-smoke.json` (operator smoke for a deployed helper bridge without putting the access code in shell history)
- `./scripts/easy_node.sh access-bridge-pilot-evidence-bundle --base-url https://HELPER_PUBLIC_DNS --path-id helper-web --code-file PRIVATE_CODE_FILE --config-json BRIDGE_SERVICE_CONFIG --deploy-pack-dir BRIDGE_DEPLOY_PACK --host-install-evidence-mode installed-host --install-dir /etc/gpm/access-bridge --systemd-unit-file /etc/systemd/system/gpm-access-bridge.service --proxy-kind caddy --proxy-config-file /etc/caddy/Caddyfile.d/gpm-access-bridge.caddy --expect-helper-id HELPER_ID --expect-org-id ORG_ID --summary-json .easy-node-logs/access-recovery-pilot/access-bridge-pilot-evidence-summary.json --provenance-sign 1 --provenance-private-key-file PROVENANCE_PRIVATE_KEY_FILE --provenance-org-id ORG_ID --provenance-org-name ORG_NAME --provenance-out .easy-node-logs/access-recovery-pilot/access-bridge-pilot-evidence.provenance.json` (one-command bridge smoke + deployment evidence + installed-host candidate evidence bundle; writes `manifest.sha256`, `<bundle>.tar.gz`, `<bundle>.tar.gz.sha256`, and a signed provenance sidecar without copying plaintext access codes or private-key files; run trusted verification before pilot handoff)
- Add `--require-mtls 1 --cacert CA.crt --client-cert CLIENT.crt --client-key CLIENT.key` to the real-helper smoke, deployment evidence, bundle, or `access-recovery-real-helper-evidence-run` command only after the public helper reverse proxy is intentionally requiring client certificates. Required-mTLS evidence must prove the client cert/key match, clientAuth EKU, fetched server leaf, distinct client/server certificate fingerprints, no-client-cert rejection on the same endpoint, and no successful HTTP 2xx no-client probe.
- `./scripts/easy_node.sh access-bridge-pilot-evidence-bundle-verify --summary-json .easy-node-logs/access-recovery-pilot/access-bridge-pilot-evidence-summary.json --provenance-json PROVENANCE_JSON --trust-store TRUST_STORE --require-trusted-provenance 1 --verification-summary-json .easy-node-logs/access_bridge_pilot_evidence_bundle_verify_summary.json --print-verification-summary-json 1` (operator handoff verification: manifest, tarball checksum sidecar, safe tar members, trusted evidence provenance, non-demo trust-store policy, semantic smoke/deployment/host evidence checks, and a roadmap verifier receipt bound to the current evidence hashes before sharing/importing; local integrity-only checks may omit provenance for diagnostics only and are never pilot/operator handoff receipts)
- Current evidence schema floors are service smoke minor `6`, deployment evidence minor `6`, pilot evidence bundle minor `8`, and verifier receipt minor `6`. Pilot-ready verifier receipts must use schema minor `6` or newer and expose explicit authority fields: `handoff_authority=true`, `authority_level=pilot_handoff`, `integrity_only=false`, and a `status_meaning` that says the receipt is pilot handoff authority. Pilot-ready verifier receipts must also show `trusted_pilot_receipt_ready=true`, `pilot_handoff_ready=true`, `pilot_handoff_criteria.bundled_child_evidence_semantic_ok=true`, `pilot_handoff_criteria.deployment_smoke_summary_sha256_matches_bundle=true`, `pilot_handoff_criteria.evidence_freshness_ok=true`, `pilot_handoff_criteria.installed_host_evidence_present=true`, real-helper HTTPS provenance from a non-demo `trust_store`, `trust_store_sha256_present=true`, `public_key_file_absent=true`, dev trust-store override disabled, smoke/deployment/host summary SHA-256 bindings, and deployment evidence bound to the exact service-smoke summary hash consumed by the bundle.
- `bash ./scripts/integration_access_recovery_examples_contract.sh` (checked-in access recovery examples sign/verify/policy/text-handoff contract)
- `go run ./cmd/gpmrecover bridge-policy --invite .easy-node-logs/access-recovery-demo/bridge-invite.signed.json --trust-store .easy-node-logs/access-recovery-demo/recovery-trust.json --signed-helper-registry .easy-node-logs/access-recovery-demo/bridge-helper-registry.signed.json` (signed bridge-invite diversity/fallback plus trusted helper allow/quarantine gate; helper registry is required by default)
- `go run ./cmd/gpmrecover bridge-policy --invite .easy-node-logs/access-recovery-demo/bridge-invite.signed.json --trust-store .easy-node-logs/access-recovery-demo/recovery-trust.json --helper-registry .easy-node-logs/access-recovery-demo/bridge-helper-registry.json --allow-unsigned-helper-registry` (diagnostic raw-registry inspection only; not trusted beta policy evidence)
- `go run ./cmd/gpmrecover bridge-service-config --invite .easy-node-logs/access-recovery-demo/bridge-invite.signed.json --trust-store .easy-node-logs/access-recovery-demo/recovery-trust.json --signed-helper-registry .easy-node-logs/access-recovery-demo/bridge-helper-registry.signed.json --out .easy-node-logs/access-recovery-demo/bridge-service-config.json` (fail-closed helper service config containing signed abuse-report/rate-limit commitments)
- `go run ./cmd/gpmrecover bridge-service-check --config .easy-node-logs/access-recovery-demo/bridge-service-config.json --path-id helper-web` (runtime preflight hook for allowing only signed, active, serviceable helper paths)
- `go run ./cmd/gpmrecover bridge-service-code-generate --code-out .easy-node-logs/access-recovery-demo/bridge-code.txt --hash-out .easy-node-logs/access-recovery-demo/bridge-code-hash.json` (generate a high-entropy bridge access code and deploy only its hash)
- `go run ./cmd/gpmrecover bridge-service-code-hash --code-file .easy-node-logs/access-recovery-demo/bridge-code.txt --out .easy-node-logs/access-recovery-demo/bridge-code-hash.json` (derive an access-code hash from an existing strong code; short/spacey diagnostic codes require `--allow-weak-code`)
- `CONFIG_HASH="$(sha256sum .easy-node-logs/access-recovery-demo/bridge-service-config.json | awk '{print $1}')"` (pin the staged service config used by the helper bridge)
- `go run ./cmd/gpmrecover bridge-service-serve --config .easy-node-logs/access-recovery-demo/bridge-service-config.json --config-sha256 "$CONFIG_HASH" --addr 127.0.0.1:18980 --rps 2 --abuse-log .easy-node-logs/access-recovery-demo/bridge-abuse.jsonl --access-code-sha256 HASH` (minimal helper HTTP wrapper with `/health`, `/bridge/{path_id}`, optional `X-GPM-Bridge-Code` gate, per-source limits, and `/abuse` logging)
- `go run ./cmd/gpmrecover bridge-service-deploy-pack --out-dir .easy-node-logs/access-recovery-demo/bridge-deploy --public-host HELPER_PUBLIC_DNS --config-sha256 "$CONFIG_HASH" --access-code-sha256 HASH` (generate env, run-wrapper, hardened systemd unit, and Caddy/nginx HTTPS examples for a helper bridge)
- `bash ./scripts/access_bridge_host_install_check.sh --deploy-pack-dir .easy-node-logs/access-recovery-demo/bridge-deploy --config-json .easy-node-logs/access-recovery-demo/bridge-service-config.json --expected-base-url https://HELPER_PUBLIC_DNS --summary-json .easy-node-logs/access-recovery-demo/bridge-host-install-check.json` (record staged/installed helper-host env, wrapper, systemd, config hash, expected public helper hostname, and proxy hardening checks)
- `go run ./cmd/gpmrecover bridge-registry-sign --helper-registry .easy-node-logs/access-recovery-demo/bridge-helper-registry.json --org-id freenews-demo --org-name "FreeNews Demo" --private-key-file .easy-node-logs/access-recovery-demo/recovery.key --out .easy-node-logs/access-recovery-demo/bridge-helper-registry.signed.json` (signed helper registry publication snapshot)
- `go run ./cmd/gpmrecover bridge-registry-verify --signed-registry .easy-node-logs/access-recovery-demo/bridge-helper-registry.signed.json --trust-store .easy-node-logs/access-recovery-demo/recovery-trust.json --out-registry .easy-node-logs/access-recovery-demo/bridge-helper-registry.verified.json` (verify signed registry before use)
- `recovery-trusted-key.txt` / `recovery-trusted-key.qr.png` from `demo-bundle`, or `go run ./cmd/gpmrecover trust-export-key --trust-store .easy-node-logs/access-recovery-demo/recovery-trust.json --org-id freenews-demo --key-id KEY_ID --text-out .easy-node-logs/access-recovery-demo/recovery-trusted-key.txt` for later trust stores (first-time trusted organization key handoff)
- `public/.well-known/gpm/` from `demo-bundle` (static publish folder for signed access pack, bridge invite, signed helper registry, trusted key, and recovery index)
- `go run ./cmd/gpmrecover fetch-publication --index-url https://freenews.example/.well-known/gpm/recovery-index.json --out-dir .easy-node-logs/access-recovery-fetched` (download same-origin static publication artifacts; verification still happens separately)
- `go run ./cmd/gpmrecover text-export --kind bridge-helper-registry-signed --in .easy-node-logs/access-recovery-demo/bridge-helper-registry.signed.json --out .easy-node-logs/access-recovery-demo/bridge-helper-registry.signed.txt` (signed helper registry chat/QR handoff)
...
- `go run ./cmd/gpmrecover bridge-registry-upsert-helper --helper-registry .easy-node-logs/access-recovery-demo/bridge-helper-registry.json --helper-id helper-second --org-ids freenews-demo --display-name "Second helper" --contact-url https://helper2.example/contact --abuse-report-url https://helper2.example/abuse --rate-limit-policy "beta cap: per-user and per-source limits enforced"` (add/update helper registry entries without hand-editing JSON)
- `go run ./cmd/gpmrecover bridge-registry-set-status --helper-registry .easy-node-logs/access-recovery-demo/bridge-helper-registry.json --helper-id helper-demo --status quarantined --reason "maintenance window"` (quarantine/re-enable helper routes without hand-editing JSON)
...
- `./scripts/easy_node.sh roadmap-next-actions-run --local-only 1 --reports-dir .easy-node-logs --print-summary-json 1` (safe local-only roadmap action runner; skips real-host/live actions unless the roadmap explicitly marks them as local-pack or not requiring real hosts; use the real-helper form below when you are intentionally running live Access Recovery pilot evidence)
- `./scripts/easy_node.sh roadmap-next-actions-run --include-id real_helper_https_evidence --access-recovery-helper-public-dns HELPER_PUBLIC_DNS --access-recovery-helper-id HELPER_ID --access-recovery-org-id ORG_ID --access-recovery-org-name ORG_NAME --access-recovery-private-code-file PRIVATE_CODE_FILE --access-recovery-bridge-service-config BRIDGE_SERVICE_CONFIG --access-recovery-bridge-deploy-pack BRIDGE_DEPLOY_PACK --access-recovery-provenance-private-key-file PROVENANCE_PRIVATE_KEY_FILE --access-recovery-trust-store TRUST_STORE --print-summary-json 1` (real-helper roadmap action runner: materializes Access Recovery operator placeholders from concrete inputs, refuses unresolved/demo/unsafe values before execution, and can also carry reports/install/proxy/systemd paths plus mTLS files when the helper requires them)
...
- `WG_ONLY_MODE` (`1` enables wireguard-only fail-closed data-plane checks across client/entry/exit roles; automatically enabled when `PROD_STRICT_MODE=1`)
- `CLIENT_WG_ONLY_MODE` (`1` enables client wireguard-only fail-closed checks for this role only)
- `ENTRY_WG_ONLY_MODE` (`1` requires entry live wireguard mode for this role only)
- `EXIT_WG_ONLY_MODE` (`1` enables exit wireguard-only fail-closed checks for this role only)
...
- `./scripts/release_generate_sbom.sh --version vX.Y.Z` (generate Go module SBOM/inventory JSON for release artifacts)
...
- `./scripts/integration_access_recovery_pivot_copy.sh` (Access Recovery docs copy contract: local rehearsal vs pilot handoff wording)
