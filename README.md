...
- `go run ./cmd/gpmrecover demo-bundle --out-dir .easy-node-logs/access-recovery-demo --org-id freenews-demo --org-name "FreeNews Demo" --helper-id helper-demo --helper-name "Demo bridge helper"` (one-command local Access Recovery demo/pilot artifacts)
...
- `./scripts/easy_node.sh access-recovery-beta-local-gate --reports-dir .easy-node-logs/access-recovery-beta-local-gate --print-summary-json 1` (focused local Access Recovery beta gate: demo/examples, browser verifier, bridge service, local/dev deployment evidence contracts, host install check, pilot bundle generation, and trusted verifier receipt contract without the legacy VPN matrix)
