...
- `go run ./cmd/gpmrecover bridge-service-serve --config .easy-node-logs/access-recovery-demo/bridge-service-config.json --addr 127.0.0.1:18980 --rps 2 --abuse-log .easy-node-logs/access-recovery-demo/bridge-abuse.jsonl` (minimal helper HTTP wrapper with `/health`, `/bridge/{path_id}`, per-source limits, and `/abuse` logging)
