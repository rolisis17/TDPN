...
- `./scripts/easy_node.sh access-recovery-local-evidence-refresh --write-canonical 1 --refresh-roadmap 1 --print-summary-json 1` refreshes local loopback rehearsal evidence; real pilot handoff still requires real helper HTTPS evidence, signed provenance, trusted verification, and a verifier receipt.
...
- `./scripts/easy_node.sh roadmap-next-actions-run --local-only 1 --reports-dir .easy-node-logs --print-summary-json 1` (safe local-only roadmap action runner; skips real-host/live actions unless the roadmap explicitly marks them as local-pack or not requiring real hosts)
