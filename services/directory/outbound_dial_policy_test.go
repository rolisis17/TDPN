...
func TestResolveSafeDialAddressRejectsPrivateDNSAddress(t *testing.T) {
	...
	target, err := resolveSafeDialAddress(
		...
		stubOutboundResolver{addrs: map[string][]net.IPAddr{
			"peer.example": {{IP: net.ParseIP("10.20.30.40")}},
		}},
		"peer.example:443",
		false,
		false,
...
func TestResolveSafeDialAddressAllowsLiteralLoopback(t *testing.T) {
	...
	target, err := resolveSafeDialAddress(context.Background(), stubOutboundResolver{}, "127.0.0.1:8081", false, true)
	if err != nil {
...
func TestResolveSafeDialAddressRejectsZoneIdentifierHost(t *testing.T) {
	...
	_, err := resolveSafeDialAddress(context.Background(), stubOutboundResolver{}, "[fe80::1%eth0]:8081", false, false)
	if err == nil {
...
func TestResolveSafeDialAddressBlocksLiteralPrivateIPByDefault(t *testing.T) {
	...
	_, err := resolveSafeDialAddress(context.Background(), stubOutboundResolver{}, "10.0.0.10:8081", false, false)
	if err == nil {
...
func TestResolveSafeDialAddressBlocksLiteralCGNATIPByDefault(t *testing.T) {
	...
	_, err := resolveSafeDialAddress(context.Background(), stubOutboundResolver{}, "100.64.0.10:8081", false, false)
	if err == nil {
...
func TestResolveSafeDialAddressDangerousOverrideAllowsPrivateDNS(t *testing.T) {
	...
	target, err := resolveSafeDialAddress(
		context.Background(),
		stubOutboundResolver{addrs: map[string][]net.IPAddr{
			"internal.peer.example": {{IP: net.ParseIP("10.2.3.4")}},
		}},
		"internal.peer.example:8443",
		true,
		false,
	)
	if err != nil {
...
func TestResolveSafeDialAddressStrictModeBlocksLiteralPrivateIP(t *testing.T) {
	...
	_, err := resolveSafeDialAddress(context.Background(), stubOutboundResolver{}, "10.0.0.10:8081", false, true)
	if err == nil {
...
func TestResolveSafeDialAddressStrictModeBlocksLiteralPrivateIPEvenWithDangerousOverride(t *testing.T) {
	...
	_, err := resolveSafeDialAddress(context.Background(), stubOutboundResolver{}, "10.0.0.10:8081", true, true)
	if err == nil {
...
func TestIsDisallowedOutboundDialIP(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "public", ip: "9.9.9.9", want: false},
	}
