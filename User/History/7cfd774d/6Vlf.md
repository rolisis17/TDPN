...
## Why this shape
Client and server in one program allowing the user to be part of the network as a relay (entry/exit) and also use the network as client. The current scaffold is built exactly that way: role flags enable any combination in one process.
