...
{
    TunDevice tun("tun0");
    NetlinkSocket netlink;
    RouteManager route(netlink);

    route.bringInterfaceUp("tun0");
    route.addIPAddress("tun0", "10.0.0.1", 24);

    UdpTransport udp(5555); // bind as server

    TunnelEngine engine(tun, udp);
    engine.run();
}
