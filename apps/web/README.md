...
- When runtime auth-verify policy requires `wallet_extension_source`, manual `Verify + Create Session` is disabled/guarded and the portal directs users to `Sign + Verify (Wallet)`; in compatibility mode or config-unavailable fallback, manual verify remains available.
