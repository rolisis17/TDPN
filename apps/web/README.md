...
- Operator apply now fails closed when `chain_operator_id` is empty; portal disables `Apply Operator Role` until Chain operator ID is populated and returns explicit strict-chain-binding guidance if triggered without it.
